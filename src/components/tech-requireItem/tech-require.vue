 ...
 * @Description:
 * @author
 * @date 2021-04-13 12:40:07
 * @edit 2021-04-13 12:40:07
*
_techReqire: {
    prop: 'techReqire',
    itemType: 'custom',
    ...
    >
      <div class="tech">
        ...
        <div class="tabs-list">
          ...
          >
            <!-- 可自定义显示 普通、手感风格、功能、印花标签-->
            <template v-if="options.tabArr">
              <el-tab-pane v-for="item in options.tabArr" :key="item.name" :label="item.label" :name="item.name" />
            </template>
            <template v-else>
...
export default {
  ...
  methods: {
    ...
    checkChange(data, check) {
      ...
      // 当前子级数据中后端没有返回'sortClassify'，所以需拿到父级中的'sortClassify'
      const parentIndex = data.parentId ? this.list.findIndex(item => { return item.id === data.parentId }) : -1
      if (!check) {
        ...
        index === -1 && this.checkValList.push({
          uuid: data.uuid, name: data.name, type: data.type || this.getType(), parent: data.parent || null, parentUuid: data.parent ? data.parent.uuid : 0, sortClassify: parentIndex === -1 ? data.sortClassify : (this.list[parentIndex].sortClassify || '')
        })
      }
    },
    ok() {
    ...
    change(check, data) {
      if (!check) {
        ...
        this.checkValList.push({
          uuid: data.uuid, name: data.name, type: data.type || this.getType(), parent: data.parent || null, parentUuid: data.parent ? data.parent.uuid : 0, sortClassify: data.sortClassify ? data.sortClassify : ''
        })
      }
    },
    // 已选标签关闭
