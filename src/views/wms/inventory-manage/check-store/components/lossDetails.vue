...
<template>
  <div class="check-all-detail">
    <header class="new-header">
      <span class="check-code">盘亏单号：{{ formDatas.checkPlanId }}</span>
    </header>
  ...
  methods: {
    ...
    async downList(data) {
      ...
      this.dataSource.forEach(val => {
        // 前端根据详情返回的库存数和盘点数相减，获得盘亏单数量
        // 盘亏单字段lossAmount由前端增加，后端未返回
        val.lossAmount = val.locationAmount - val.checkAmount
