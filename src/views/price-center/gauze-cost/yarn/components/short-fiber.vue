...
import { getSelectData, getChengFen, getZhanBi, getColorNo, getYarnBrand } from '@/views/price-center/common/basisApi'
...
export default {
    ...
    const searchData = [
      ...
      },
      {
        ...
        submitHandle: params => {
          ...
          this.getShortPageFn(params, true)
        },
        resetHandle: () => {
  ...
  methods: {
    async editFn(id) {
      const data = (await getShortDetail({ id })).data || {}
      this.oldForm = data
    ...
    addFn() {
      ...
      this.PopDialogHandle('新增短纤')
    },
    // 获取数据
    async getShortPageFn(data = {}, isSearch) {
      const res = await getShortPage({ ...this.searchFormDatas, ...data })
    ...
    async enableStatus(data, scope) {
      await enabledShort(data).then(e => this.$message.success('更新状态成功')).catch(() => {
        scope.row.status = data.status === 0 ? 1 : 0
      })
      this.fetchLog(this.getLogMessages('UPDATE', '/priceCenter/spunyarnInfo/status'), data, JSON.stringify({ beforeText: `在'价格中心-纱成本-纱线'${data.status ? '启用' : '禁用'}一条数据`, beforeCode: data }))
    },
    // 新增或者修改印花
    addOrEdit(params, go) {
      this.$refs.popForm.$refs.popComponents.form.validate(async valid => {
        if (valid) {
          if (this.type === 'add') {
            await addShort(params)
            this.fetchLog(this.getLogMessages('INSERT', '/priceCenter/spunyarnInfo/add'), params, JSON.stringify({ beforeText: `在'价格中心-纱成本-纱线,新增一条记录`, beforeCode: params }))
          } else if (this.type === 'edit') {
            await editShort(params)
            ...
            flag && this.fetchLog(this.getLogMessages('UPDATE', '/priceCenter/spunyarnInfo/modify'), params, JSON.stringify({ beforeText: `在'价格中心-纱成本-纱线,将${JSON.stringify({ 'id': params.id })}`, afterText: '修改为', ...{ beforeCode, afterCode }}))
          }
          this.$message.success('保存成功')
          this.getShortPageFn({ pageSize: this.pagination.pageSize || 20, pageNum: this.pagination.pageNum || 1 })
          this.$set(this.popOptions, 'formDatas', {})
          if (go) {
            this.PopDialogHandle()
          }
    ...
    // 删除
    async deleteFn(data = {}) {
      await deleteShort(data)
      this.$message.success('删除成功')
      this.getShortPageFn({ pageSize: this.pagination.pageSize || 20, pageNum: this.pagination.pageNum || 1 })
      this.fetchLog(this.getLogMessages('DEL', '/priceCenter/spunyarnInfo/delete'), data, JSON.stringify({ beforeText: `在'价格中心-纱成本-纱线'删除一条记录`, beforeCode: data }))
    },
    // 设置弹框下拉数据
    setSelectData() {
      ...
      const arr = [
        ...
        [getSelectData, 1, { categoryId: 1 }],
        [getChengFen, 2],
        [getZhanBi, 3],
        [getSelectData, 4, { categoryId: 2 }],
        [getSelectData, 5, { categoryId: 3 }],
        [getSelectData, 6, { categoryId: 6 }],
        [getColorNo, 7, { status: '1' }],
        [getYarnBrand, 8, { status: '1' }]
      ]
      arr.forEach(async e => {
