<template>
  <div class="paymentPage">
    ...
    <!-- 新增菜单按钮 -->
    <div class="button">
      <el-button v-permission="['price:supplerColor:add']" type="primary" size="small" icon="el-icon-plus" @click="addList">新增</el-button>
    </div>
    <!-- table表格 -->
...
export default {
  ...
  data() {
    ...
    const columns = [
      ...
      },
      {
        ...
        componentsOptions: {
          permitTag: 'price:supplerColor:status',
          changeStatus: (scope) => {
    ...
    let popOperatesDataSource = [
      {
        label: '编辑',
        permitTag: ['price:supplerColor:modify'],
        isShow: true,
        handle: scope => {
          this.type = 'edit'
          this.PopDialogHandle('编辑供应商色号名称')
      ...
      },
      {
        ...
        isShow: modifyIsShow,
        style: { 'color': '#FE4949' },
        permitTag: ['price:supplerColor:delete'],
        handle: scope => {
  ...
  methods: {
    addList() {
      ...
      this.PopDialogHandle('新增供应商色号名称')
    },
    PopDialogHandle(val) {
    ...
    addOrEdit(params = {}, go) {
      this.$refs.popForm.$refs.popComponents.form.validate(async valid => {
        if (valid) {
          if (this.type === 'add') {
            // 新增
            await supplierColorAdd(params)
            this.fetchLog(this.getLogMessages('INSERT', '/priceCenter/supplierColor/add'), params, JSON.stringify({ beforeText: `在'价格中心-供应商色号名称,新增一条记录`, beforeCode: params }))
          } else if (this.type === 'edit') {
            // 修改
            const obj = {}
            const pamraskey = ['id', 'supplierColorName']
            pamraskey.forEach(v => {
              obj[v] = params[v]
            })
            await supplierColorModify(obj)
            ...
            flag && this.fetchLog(this.getLogMessages('UPDATE', '/priceCenter/supplierColor/modify'), obj, JSON.stringify({ beforeText: `在'价格中心-供应商色号名称,将${JSON.stringify({ 'id': obj.id })}`, afterText: '修改为', ...{ beforeCode, afterCode }}))
          }
    ...
    async getRulePage(data = {}) {
      const res = await supplierColorPage({ ...this.formDatas, ...data })
    ...
    async editList(data = {}, code) {
      const res = await supplierColorDetail(data)
      if (!res || res.code !== 200) {
        return false
      }
    ...
    // 删除
    async deleteData(id) {
      const res = await supplierColorDelete({ id: id })
      if (res.code !== 200) {
        return false
      }
      this.$message({
      ...
      this.getRulePage({ pageNum: this.pagination.page || 1, pageSize: this.pagination.pageSize || 20 })
      this.fetchLog(this.getLogMessages('DEL', '/priceCenter/supplierColor/delete'), { id }, JSON.stringify({ beforeText: `在'价格中心-供应商色号名称'删除一条记录`, beforeCode: { id }}))
    },
    // 启用/禁用
    async changeEnable(data, scope) {
      await supplierColorEnabled(data).then(e => this.$message.success('更新状态成功')).catch(() => {
        scope.row.status = data.status === 0 ? 1 : 0
      })
      this.fetchLog(this.getLogMessages('UPDATE', '/priceCenter/supplierColor/status'), data, JSON.stringify({ beforeText: `在'价格中心-供应商色号名称'${data.status ? '启用' : '禁用'}一条数据`, beforeCode: data }))
    }
  }
}
</script>
