<template>
  <div class="paymentPage">
    ...
    <!-- 新增菜单按钮 -->
    <div class="button">
      <el-button v-permission="['price:testStandard:add']" type="primary" size="small" icon="el-icon-plus" @click="addList">新增</el-button>
    </div>
    <!-- table表格 -->
...
<script>
import { testStandardPage, testStandardAdd, testStandardDetail, testStandardModify, testStandardDelete, testStandardEnabled } from './api/index.js'
...
export default {
  ...
  data() {
    ...
    const columns = [
      ...
      },
      {
        ...
        componentsOptions: {
          permitTag: 'price:testStandard:status',
          changeStatus: (scope) => {
    ...
    let popOperatesDataSource = [
      ...
      },
      {
        ...
        isShow: modifyIsShow,
        style: { 'color': '#FE4949' },
        permitTag: ['price:testStandard:delete'],
        handle: scope => {
  ...
  methods: {
    ...
    addOrEdit(params = {}, go) {
      this.$refs.popForm.$refs.popComponents.form.validate(async valid => {
        if (valid) {
          if (this.type === 'add') {
            // 新增
            await testStandardAdd(params)
            this.fetchLog(this.getLogMessages('INSERT', '/priceCenter/testStandard/add'), params, JSON.stringify({ beforeText: `在'价格中心-测试标准,新增一条记录`, beforeCode: params }))
          } else if (this.type === 'edit') {
            // 修改
            const obj = {}
            const pamraskey = ['id', 'testStandardName']
            pamraskey.forEach(v => {
              obj[v] = params[v]
            })
            await testStandardModify(obj)
            ...
            flag && this.fetchLog(this.getLogMessages('UPDATE', '/priceCenter/testStandard/modify'), obj, JSON.stringify({ beforeText: `在'价格中心-测试标准,将${JSON.stringify({ 'id': obj.id })}`, afterText: '修改为', ...{ beforeCode, afterCode }}))
          }
          this.$set(this.popOptions, 'formDatas', {})
    ...
    async getRulePage(data = {}) {
      const res = await testStandardPage({ ...this.formDatas, ...data })
    ...
    async editList(data = {}, code) {
      const res = await testStandardDetail(data)
      if (!res || res.code !== 200) {
        return false
      }
    ...
    // 删除
    async deleteData(id) {
      const res = await testStandardDelete({ id: id })
      if (res.code !== 200) {
        return false
      }
      this.$message({
      ...
      this.getRulePage({ pageNum: this.pagination.page || 1, pageSize: this.pagination.pageSize || 20 })
      this.fetchLog(this.getLogMessages('DEL', '/priceCenter/testStandard/delete'), { id }, JSON.stringify({ beforeText: `在'价格中心-测试标准'删除一条记录`, beforeCode: { id }}))
    },
    // 启用/禁用
    async changeEnable(data, scope) {
      await testStandardEnabled(data).then(e => this.$message.success('更新状态成功')).catch(() => {
        scope.row.status = data.status === 0 ? 1 : 0
      })
      this.fetchLog(this.getLogMessages('UPDATE', '/priceCenter/testStandard/status'), data, JSON.stringify({ beforeText: `在'价格中心-测试标准'${data.status ? '启用' : '禁用'}一条数据`, beforeCode: data }))
    }
  }
}
</script>
