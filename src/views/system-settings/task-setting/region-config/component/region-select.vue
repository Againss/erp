<template>
  <div v-if="componentsOptions.hackReset" class="form-region-select">
    ...
    <div class="select-contanier">
      ...
      <div class="factory-content">
        <div class="left">
          <ul>
            <li v-for="(item, index) in componentsOptions.factoryData" :key="index" :title="item.supplierName">
              <el-checkbox
        ...
        <div class="right">
          ...
          <ul class="selcted">
            <li v-for="(item, index) in componentsOptions.selectData" :key="index" :title="item.supplierName">
              {{ item.supplierName }}
...
<script>
import { areasOption } from '@/views/price-center/common/basisApi'
export default {
    ...
    const searchData = [
      {
        ...
        change: (value, form, formDatas, setFormDatas) => {
          ...
          this.formDatas.provinceCode = value
          this.formDatas.cityCode = ''
          this.getCity(value)
        }
      },
      {
        ...
        change: (value, form, formDatas) => {
          this.formDatas.cityCode = value
          this.search()
        },
        visibleChange: (value, form, formDatas, setFormDatas) => {
          if (value) {
            console.log('000', value, form, formDatas)
          }
        }
      },
      {
  ...
  methods: {
    // 获取省份
    async getProvince() {
      const res = await areasOption({ parentId: 0 })
      console.log(res)
      this.searchData[0].dataSource = [{ label: '全部', value: '' }, ...res]
      this.$set(this.formDatas, 'parentId', '')
    },
    // 获取城市
    async getCity(id) {
      const res = await areasOption({ parentId: id })
      console.log(res)
      this.searchData[1].dataSource = [{ label: '全部', value: '' }, ...res]
      this.search()
    },
