 ...
 * @Author: Againss
 * @LastEditTime: 2021-07-08 09:49:17
...
export default {
      ...
      content: [
        {
          ...
          onSuccess: (res, file) => {
            ...
            res.data && res.data.code && res.data.code.length && this.fetchLog(
              this.getLogMessages('IMPORT', `${configData.productRequestPrefix}/api/embryo/import`),
              file,
              JSON.stringify({
                beforeText: `在'产品目录库-产品管理-坯布'批量导入成功`,
          ...
          action: `${configData.productRequestPrefix}/api/embryo/import`,
  ...
  methods: {
     ...
     */
    downloadTemplate() {
      location.href = `https://oss-sfabric.sfabric.com/product-catalog-demo/demo/embryo_import.xlsx?T=${new Date().getTime()}`
    },
    /**
