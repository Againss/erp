<!--
 * @Author: Sanmao
 * @Date: 2020-12-23 10:35:01
 * @LastEditors: zhengjin
 * @LastEditTime: 2021-07-08 09:48:53
 * @Descripttion: 色/花布批量导入页面
-->
<template>
...
export default {
      ...
      content: [
        {
          ...
          onSuccess: (res, file) => {
            ...
            res.data && res.data.code && res.data.code.length && this.fetchLog(
              this.getLogMessages('IMPORT', `${configData.productRequestPrefix}/api/cloth/productFabric/import`),
              file,
              JSON.stringify({
                beforeText: `在'产品目录库-产品管理-色布列表'批量导入成功`,
          ...
          action: `${configData.productRequestPrefix}/api/cloth/productFabric/import`,
  ...
  methods: {
     ...
     */
    downloadTemplate() {
      location.href = `https://oss-sfabric.sfabric.com/product-catalog-demo/demo/productFabricImport.xls?T=${new Date().getTime()}`
    },
    /**
