 ...
 * @LastEditTime: 2021-07-08 09:49:03
 * @Descripttion: spu批量导入页面
-->
<template>
...
export default {
      ...
      content: [
        {
          ...
          onSuccess: (res, file) => {
            ...
            res.data && res.data.code && res.data.code.length && this.fetchLog(
              this.getLogMessages('IMPORT', `${configData.productRequestPrefix}/api/spu/import`),
              file,
              JSON.stringify({
                beforeText: `在'产品中心-SPU管理-SPU'批量导入成功`,
          ...
          action: `${configData.productRequestPrefix}/api/spu/import`,
  ...
  methods: {
     ...
     */
    downloadTemplate() {
      location.href = `https://oss-sfabric.sfabric.com/product-catalog-demo/demo/spuImport.xls?T=${new Date().getTime()}`
    },
    /**
