<!--
 * @Author: Sanmao
 * @Date: 2020-12-22 10:58:24
 * @LastEditors: zhengjin
 * @LastEditTime: 2021-07-08 09:51:11
 * @Descripttion:化学纤维批量导入页面
-->
<template>
...
export default {
      ...
      content: [
        {
          ...
          onSuccess: (res, file) => {
            ...
            res.data && res.data.codes && res.data.codes.length && this.fetchLog(
              this.getLogMessages('IMPORT', `${configData.productRequestPrefix}/api/yarn/chemicalFiber/import`),
              file,
              JSON.stringify({
                beforeText: `在'产品目录库-纱线管理-化学纤维列表'批量导入成功`,
          ...
          action: `${configData.productRequestPrefix}/api/yarn/chemicalFiber/import`,
  ...
  methods: {
     ...
     */
    downloadTemplate() {
      location.href = `https://oss-sfabric.sfabric.com/product-catalog-demo/demo/ChemicalFiberImportModule.xlsx?T=${new Date().getTime()}`
    },
    /**
