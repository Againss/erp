<template>
  <div class="customer-level">
    <div class="button">
      <el-button v-permission="['userCenter:sysUser:add']" size="small" type="primary" @click="createCustomerLevel">新建</el-button>
    </div>
    <!-- table表格 -->
...
<script>
import { CustomerLevel } from './components/index'
import { PublicCrm } from '@/views/crm/public/index'
export default {
  mixins: [CustomerLevel, PublicCrm],
  data() {
    // 公用验证正则
    ...
    const pagination = {
      ...
      currentChange: val => {
        ...
        this.getCustomerLevelPage({ pageSize: this.pagination.pageSize || 20, pageNum: val })
      },
      sizeChange: val => {
        // console.log('sizeChange', this.pagination)
        this.getCustomerLevelPage({ pageNum: this.pagination.pageNum || 1, pageSize: val })
      }
    }
    let handleConf = [
      {
        ...
        handle: params => {
          ...
          this.PopDialogHandle(this.popOptions, '编辑客户级别选项')
          this.getCustomerLevelInfo({ id: params.row.id })
        }
      },
      {
        label: '删除',
        ...
        isShow: true,
        handle: params => {
          this.$confirm(' 你确认要删除该客户级别选项吗？', '提示', {
            ...
            .then(() => {
              this.deleteCustomerLevel({ id: params.row.id })
            })
            .catch(() => {
              this.$message({
                type: 'info',
    ...
    const popOptions = {
      ...
      ok: params => {
        ...
        this.addOrEditCustomerLevel(params)
      },
      cancel: params => {
