<template>
  <div class="clue-source">
    <div class="button">
      <el-button v-permission="['userCenter:sysUser:add']" size="small" type="primary" @click="createClueSource">新建</el-button>
    </div>
    <!-- table表格 -->
...
<script>
import { ClueSource } from './components/index'
import { PublicCrm } from '@/views/crm/public/index'
export default {
  mixins: [ClueSource, PublicCrm],
  data() {
    // 公用验证正则
    ...
    const pagination = {
      ...
      currentChange: val => {
        ...
        this.getClueSourcePage({ pageSize: this.pagination.pageSize || 20, pageNum: val })
      },
      sizeChange: val => {
        // console.log('sizeChange', this.pagination)
        this.getClueSourcePage({ pageNum: this.pagination.pageNum || 1, pageSize: val })
      }
    }
    let handleConf = [
      {
        ...
        handle: params => {
          ...
          this.PopDialogHandle(this.popOptions, '编辑线索来源选项')
          this.getClueSourceInfo({ id: params.row.id })
        }
      },
      {
        label: '删除',
        ...
        isShow: true,
        handle: params => {
          this.$confirm(' 你确认要删除该线索来源选项吗？', '提示', {
            ...
            .then(() => {
              this.deleteClueSource({ id: params.row.id })
            })
            .catch(() => {
              this.$message({
                type: 'info',
    ...
    const popOptions = {
      ...
      ok: params => {
        ...
        this.addOrEditClueSource(params)
      },
      cancel: params => {
