<template>
  <div class="contract-view-all-page">
    <div class="contractHeader">
      <span v-if="contractFormDatas.name" class="contractText">{{ contractFormDatas.name }}</span>
...
export default {
  mixins: [PublicCrm, ContractRelevant],
  data() {
    // 公用验证正则
  ...
  methods: {
    viewAllAdd() {
      ...
      const contractFormDatas = this.contractFormDatas
      this.$set(this.contractPopOptions.content[1], 'dataSource', [{ label: contractFormDatas.customerName, value: contractFormDatas.customerId }])
      this.$set(this.contractPopOptions.content[2], 'dataSource', [{ label: contractFormDatas.name, value: contractFormDatas.id }])
      this.setFormDatas(this.contractPopOptions, { customerId: contractFormDatas.customerId, businessOpportunityId: contractFormDatas.id })
    },
    // 获取商机详情
    async getBusinessOpportunityInfo(data = {}) {
      const res = await businessOpportunityInfo(data)
      console.log('详情', res)
      this.contractFormDatas = res.data
