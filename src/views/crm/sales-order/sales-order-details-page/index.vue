<template>
  <div class="sales-order-details-page">
    <div class="salesOrderHeader">
      <span class="salesOrderText">销售订单申请</span>
      <div class="details-button">
        <el-button v-if="salesOrderStaus === 'D'" size="mini" @click="submitApplication">提交申请</el-button>
        <el-button v-if="salesOrderStaus === 'D'" size="mini" @click="detailEdit">编辑</el-button>
        <el-button v-if="salesOrderStaus === 'D'" size="mini" type="danger" plain @click="detailDelete">删除</el-button>
        <el-button v-if="salesOrderStaus === 'P' || salesOrderStaus === 'S'" size="mini" @click="withdraw">取消申请</el-button>
        <el-button size="mini" @click="changeOwnerClick">更改所有人</el-button>
      </div>
    </div>
    <!-- 状态控制 -->
    <div style="margin: 30px 0 0 0;">
      <div v-if="salesOrderStaus !== 'R'" class="salesOrderStatusLine">
      ...
      <div v-else class="closeStatus">
        ...
        <div class="salesOrderClosed">销售订单已取消</div>
        <div class="closeDottedLine" />
      </div>
    </div>
    <!-- 搜索表单 -->
...
import { SalesOrderRelevant } from '../components/relevantIndex.js'
...
export default {
  ...
  data() {
    ...
    const salesOrderPopOptions = {
      ...
      ok: params => {
        ...
        this.editSalesOrder({ ...params, brandId, brandName, sellerTeamId, sellerTeamName, orderFilePath, styleFilePath, otherFilePath, totalAmount, payCondition, payConditionName, priceTerm, priceTermName }, true)
      },
      cancel: params => {
    ...
    const withdrawPopOptions = {
      ...
      ok: params => {
        console.log(params)
        this.salesOrderModifyStatus({ customerRequireId: this.routeId, status: 'R', ...params }, '取消申请成功')
      },
      cancel: params => {
  ...
  computed: {
    ...
    modelDevelopId() {
      return this.detailsFormDatas.modelDevelopId
    },
    brandId() {
  ...
  watch: {
    ...
    modelDevelopId(newVal, oldNew) {
      this.$set(this.baseSearchData[2].componentsOptions, 'url', `/crm/model-develop/model-develop-details-page/${newVal}`)
    },
  ...
  created() {
    this.routeId && this.getSalesOrderInfoDisplay({ customerRequireId: this.routeId })
  ...
  methods: {
    ...
    detailEdit() {
      ...
      this.getSalesOrderInfo({ customerRequireId: this.routeId })
    },
    ...
    detailDelete() {
      this.$confirm(' 你确认要删除该销售订单申请吗？', '提示', {
        ...
        .then(() => {
          this.deleteSalesOrder({ customerRequireId: this.routeId }, true)
        })
        .catch(() => {
          this.$message({
            type: 'info',
    ...
    //   console.log(111)
      this.$confirm('确定提交申请吗?', '提示', {
        ...
        this.salesOrderModifyStatus({ customerRequireId: this.routeId, status: 'S' }, '提交申请成功')
      }).catch(() => {
        this.$message({
          type: 'info',
          message: '已取消'
    ...
    onGoingStatusChange(value) {
      console.log(value)
      if (this.salesOrderStaus !== 'T') {
        if (value === 'T') {
          this.$message.warning('请点击右上角转化按钮')
        } else if (this.salesOrderStaus !== value) {
          this.salesOrderStausModify({ salesOrderStaus: value })
        }
      }
    },
    ...
    closedStateChange() {
      this.$confirm('此操作将关闭线索进度, 是否继续?', '提示', {
        ...
        if (this.salesOrderStaus !== 'C') {
          this.salesOrderStausModify({ salesOrderStaus: 'C' })
        }
      }).catch(() => {
        this.$message({
          type: 'info',
