<template>
  <div class="computer-technology">
    ...
    <div class="pin-number">
      <div class="pin-title" style="display: flex;align-items: center;">
        <div class="pin-info">
          <span><i>*</i>排针图</span>
          <span>（代表符号：0 1 2 3 4 ）</span>

        </div>
        <div style="margin-right: 53px;display: flex;align-items: center;" :class="isEdit === 'detail' ? 'technology-box' : 'edit-box'">
        ...
        <span style="font-size: 14px;color: #474747;margin-left: auto;">对针类型：</span>
        <el-select
          v-model="order.stitchType"
          style="margin-right: 56px;"
          :disabled="isEdit==='detail'?true:false"
          :readonly="isEdit==='detail'?true:false"
          placeholder="请选择"
        >
          <el-option value="" />
          <el-option value="罗纹对针">罗纹对针</el-option>
          <el-option value="双面对针">双面对针</el-option>
        </el-select>
      </div>
      <div class="pin-table">
...
<script>
import { tmWeavingTechList } from '../api/index.js'
export default {
  props: {
    isEdit: {
      type: String
  ...
  data() {
    ...
    const check10 = this.$getRules({ type: 'number', pattern: /^(([1-9]{1}\d{0,9})|(0{1}))(\.\d{1,2})?$/, message: '请填最多10位整数，可保留2位小数', trigger: ['blur'] })
    ...
    const itemStyle = {
      'width': 'calc(25% - 10px)', 'padding-left': '0', 'margin-bottom': '16px', 'margin-right': '0'
    }
    const infoDataSource = {
      ...
      _typeInfo: {
        ...
        renderContent: (value, options, form, formDatas, setFormDatas, scope) => {
          if (value && value.label) {
            return `<span title=${value.label}>${value.label}</span>`
          } else {
            return ''
          }
        }
      ...
      _module: {
        ...
        label: '开机模数:',
        maxlength: 14,
        // rules: [{ validator: check10, trigger: 'blur' }],
        rules: [check10],
        // disabled: true,
        // readonly: true,
        placeholder: '请输入开机模数'
  ...
  methods: {
    ...
    async getTmWeavingTechList() {
      const res = (await tmWeavingTechList({ techId: this.techId })).data || {}
      const dataObj = {
        'C': [],
        'K': [],
        'R': [],
      ...
      this.$set(this.infoDataSource._typeInfo, 'dataSource', dataObj['C'])
      this.$set(this.infoDataSource._packageWayInfo, 'dataSource', dataObj['P'])
      this.$set(this.infoDataSource._loomTypeInfo, 'dataSource', dataObj['K'])
      this.$set(this.infoDataSource._loomTurnInfo, 'dataSource', dataObj['R'])
    },
    initData(val) {
      // if (val === 1) {
      //   for(let key in this.infoDataSource){
      //     this.$set(this.infoDataSource[key], 'readonly', false)
      //     this.$set(this.infoDataSource[key], 'disabled', false)
      //     this.$set(this.infoDataSource[key], 'placeholder', '')
      //   }
      // } else {
      //   for(let key in this.infoDataSource){
      //     this.$set(this.infoDataSource[key], 'readonly', true)
      //     this.$set(this.infoDataSource[key], 'disabled', true)
      //     this.$set(this.infoDataSource[key], 'placeholder', '')
      //   }
      // }
    },
...
.computer-technology {
  ...
  padding: 0 20px;
  // margin: 0 20px;
  .technology-box .el-input.is-disabled .el-input__inner {
  ...
  .technology-box  .el-input__inner{
    background-color: transparent;
    border: none;
    color: #777;
    text-overflow: ellipsis;
  ...
  .technology-box .el-input__suffix {
    display: none;
  }
