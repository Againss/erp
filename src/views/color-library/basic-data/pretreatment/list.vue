 ...
 * @LastEditTime: 2021-06-22 16:21:19
...
<template>
  <div class="categoriesPage product-list-content">
    ...
    <div class="button">
      <el-button
        v-permission="['color:Api:ColorBd:Pretreatment:store']"
        size="small"
        type="primary"
        icon="el-icon-plus"
        @click="addList"
...
export default {
  ...
  data() {
    ...
    const columns = [
      ...
      },
      {
        ...
        componentsOptions: {
          permitTag: 'color:Api:ColorBd:Pretreatment:enable',
          changeStatus: (scope) => {
            this.changeEnable(
              {
                id: scope.row.id,
    ...
    let popOperatesDataSource = [
      {
        label: '审核',
        permitTag: ['color:Api:ColorBd:Pretreatment:approve'],
        isShow: (scope) => {
      ...
      {
        label: '编辑',
        permitTag: ['color:Api:ColorBd:Pretreatment:update'],
        isShow: true,
        handle: (scope) => {
      ...
      {
        label: '删除',
        permitTag: ['color:Api:ColorBd:Pretreatment:destroy'],
        isShow: (scope) => {
          return scope.row.status.code === 'B'
        },
        handle: params => {
          this.$confirm('确认删除当前数据吗？', '提示', {
            ...
            .then(async() => {
              ...
              this.fetchLog(
                this.getLogMessages('DEL', '/color/api/colorBd/pretreatment/destroy', 'color'),
                { id: params.row.id, name: params.row.name },
                JSON.stringify({
                  beforeText: `在'色号花号库-基础数据管理-前处理'删除一条记录`,
  ...
  methods: {
    ...
    async addData(data = {}) {
      ...
      this.fetchLog(
        this.getLogMessages('INSERT', '/color/api/colorBd/pretreatment/store/store', 'color'),
        { id: data.id, name: data.name },
        JSON.stringify({
          beforeText: `在'色号花号库-基础数据管理-前处理'新增一条记录`,
    ...
    async addUpdate(data = {}) {
      ...
      this.fetchLog(
        this.getLogMessages('UPDATE', '/color/api/colorBd/pretreatment/update', 'color'),
        { id: data.id, name: data.name },
        JSON.stringify({
          beforeText: `在'色号花号库-基础数据管理-前处理'编辑一条记录`,
          beforeCode: { id: data.id, name: data.name }
        })
      )
      this.$set(this.popOptions, 'visible', false)
    ...
    async approveData(id, scope) {
      ...
      this.fetchLog(
        this.getLogMessages('UPDATE', '/color/api/colorBd/pretreatment/approve', 'color'),
        { id: scope.row.id, code: scope.row.code },
        JSON.stringify({
          beforeText: `在'色号花号库-基础数据管理-前处理'审核一条记录`,
    ...
    changeEnable: debounce(
      async function(data = {}, scope) {
        ...
        if (res.code === 200) {
          this.fetchLog(
            this.getLogMessages('UPDATE', '/color/api/colorBd/pretreatment/enable', 'color'),
            { id: scope.row.id, name: scope.row.name },
            JSON.stringify({
              beforeText: `在"色号花号库-基础数据管理-前处理"${data.enabled === 'Y' ? '启用' : '禁用'}一条记录`,
              beforeCode: { id: scope.row.id, name: scope.row.name }
            })
          )
          this.$message({
