<template>
  <div class="wrap-data-list">
    ...
    <div class="wrap-btns">
      <div class="button">
        <el-button
          v-permission="['bmpAps:apsPrintDay:add']"
          type="primary"
          size="small"
          @click="handleAdd"
        >新建</el-button>
...
export default {
  ...
  data() {
    ...
    const columns = [
      ...
      },
      {
        ...
        componentsOptions: {
          permitTag: 'bmpAps:apsPrintDay:status',
          changeStatus: (scope) => {
    ...
    let popOperatesDataSource = [
      {
        label: '编辑',
        isShow: true,
        permitTag: ['bmpAps:apsPrintDay:modify'],
        handle: (scope) => {
  ...
  methods: {
    ...
    async enableStatus(data) {
      const res = await changeApsPrintDay(data)
      if (res.code === 200) {
        this.$message({
          message: '更新状态成功',
          type: 'success'
      ...
      this.fetchLog(this.getLogMessages('UPDATE', '/baseData/apsPrintDay/status'), data, JSON.stringify({ beforeText: `在'产能中心-生产天数设置-印花'${data.status === 0 ? '启用' : '停用'}一条数据`, beforeCode: data }))
      this.getRulePage({
    ...
    addOrEdit(params = {}, go) {
      this.$refs.popForm.$refs.popComponents.form.validate(async(valid) => {
        if (valid) {
          ...
          // }
          if (this.type === 'edit') {
            await modifyApsPrintDay(params)
            ...
            flag &&
              this.fetchLog(
                this.getLogMessages('UPDATE', '/baseData/apsPrintDay/modify'),
                params,
                JSON.stringify({
                  beforeText: `在'产能中心-生产天数设置-印花'将${JSON.stringify(
                    { id: params.id }
                  )}`,
                  afterText: '修改为',
            ...
            await addApsPrintDay(params)
            this.fetchLog(
              this.getLogMessages('INSERT', '/baseData/apsPrintDay/add'),
              params,
              JSON.stringify({
                beforeText: `在'产能中心-生产天数设置-印花'新增一条记录`,
                beforeCode: params
              })
            )
            this.getRulePage()
    ...
    async getRulePage(data = {}, isSearch) {
      const res = await apsPrintDayList({
        ...data
      })
    ...
    async selectDatasource() {
      const res = await requirementsPrintingList()
      ...
      console.log('this.popOptions.content._param', this.popOptions.content._param)
    }
  }
}
</script>
