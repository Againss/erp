 ...
 * @Date: 2020-12-10 16:18:04
 * @LastEditors: OBKoro1
 * @LastEditTime: 2021-07-05 19:24:51
-->
<template>
  <div class="guest-application2">
    <!-- 搜索表单 -->
    ...
    <div class="tabchange">
      <div class="tabchange_he">
        ...
        <div class="createbutton">
          <el-button
            v-permission="['soPrint:printApply:add']"
            size="small"
            type="primary"
            @click="addHandler"
          >新建</el-button>
...
<script>
import { pageList, sysDataGroupApprove, sysCancelData } from './api/index.js'
...
export default {
  ...
  data() {
    ...
    let popOperatesDataSource = [
      {
        label: '编辑',
        permitTag: ['soPrint:printApply:modify'],
        isShow: (scope) => {
          return scope.row.status === 0
        },
        handle: scope => {
          this.$router.push(// 跳转到详情页
            `/customer-order/analysis-application/printing-application/detail/${scope.row.id}/edit`
          )
        }
      },
      {
        label: '完结',
        permitTag: ['soPrint:printApply:finish'],
        isShow: (scope) => {
      ...
      {
        label: '取消生产',
        permitTag: ['soPrint:printApply:cancel'],
        isShow: (scope) => {
          return scope.row.status === 1
        },
        handle: scope => {
    ...
    const columns = [
      ...
      },
      {
        ...
        handle: (scope) => {
          this.$router.push(
            `/customer-order/analysis-application/printing-application/detail/${scope.row.id}/show`
          )
        },
        style: {
  ...
  methods: {
    ...
    // 新建
    addHandler() {
      this.$router.push('/customer-order/analysis-application/printing-application/create')
    },
    // 是否完结订单
    ...
    // 取消生产
    async cancelData(id) {
      const res = await sysCancelData({ id: id })
      if (res.code !== 200) { return false }
      this.$message({
        type: 'success',
        message: '取消生产成功!'
