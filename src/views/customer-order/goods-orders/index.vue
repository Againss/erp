 ...
 * @LastEditors: OBKoro1
 * @LastEditTime: 2021-07-27 17:41:50
-->
<template>
        ...
        <div class="createbutton">
          <div class="createbutton_btn">
            <div class="cr_btn_left">
              <el-button
                v-permission="['customerOrder:saleOrder:add']"
                size="small"
                type="primary"
                @click="addHandler"
              >新建</el-button>
...
export default {
  ...
  data() {
    ...
    const columns = {
      ...
      _code: {
        ...
        handle: (scope) => {
          this.$router.push(
            `/customer-order/goods-orders/detail/${scope.row.id}/show`
          )
          console.log(scope.row)
        },
        formater: scope => scope.row && scope.row.code || '-',
        style: {
    ...
    const allColumns = {
      ...
      _currencyName: {
        ...
        minWidth: '150',
        showOverflowTooltip: true,
        formater: scope => scope.row && scope.row.currencyName || '-'
      },
      _pricingTermName: {
    ...
    let popOperatesDataSource = [
      {
        label: '编辑',
        permitTag: ['customerOrder:saleOrder:modify'],
        isShow: (scope) => {
          return scope.row.status === '0' || scope.row.status === '4'
        },
        handle: (scope) => {
          console.log(scope)
          this.$router.push(`/customer-order/goods-orders/detail/${scope.row.id}/edit`)
        }
      },
      {
        label: '变更',
        permitTag: ['customerOrder:saleOrder:change'],
        isShow: (scope) => {
          return scope.row.status === '2'
        },
        handle: (scope) => {
          console.log(scope)
          this.$router.push(`/customer-order/goods-orders/detail/${scope.row.id}/edit?statusIndex=${scope.row.status}`)
        }
      },
      {
        label: '解hold',
        permitTag: ['customerOrder:saleOrder:removeHold'],
        isShow: (scope) => {
        ...
        handle: (scope) => {
          ...
          // })
          //   .then(() => {
          //     this.deletHold({ orderId: scope.row.orderId })
          //   })
          //   .catch(() => {
          //     this.$message({
          //       type: 'info',
    ...
    return {
      ...
      isArrayData: [],
      statusList: {
        'all': { value: '-1', label: '全部', count: 0 },
        'waitSubmit': { value: '0', label: '待提交', count: 0 },
        'waitAudit': { value: '1', label: '待审核', count: 0 },
        'processing': { value: '2', label: '进行中', count: 0 },
        'finished': { value: '3', label: '已完结', count: 0 },
  ...
  methods: {
    ...
    //   const selArray = filters[Object.keys(filters)]
    //   console.log(this.searchFormDatas)
    //   this.list({...this.searchFormDatas,status:selArray})
    // },
    // 销售团队下拉
    async getOrg() {
    ...
    // 订单类型下拉
    async orderTypy() {
      const res = await orderTypeselect({ fieldName: '订单类型', pageName: '大货订单' })
      if (res.code !== 200) {
        return false
      }
      let orderTypeData = []
    ...
    // },
    addHandler() {
      this.$router.push(`/customer-order/goods-orders/create`)
    },
