 ...
 * @LastEditors: OBKoro1
 * @LastEditTime: 2021-03-31 20:21:43
-->
<template>
  <el-dialog
    title="选择色布/印花编码"
  ...
  methods: {
    ...
    getClothSelect() {
      this.getCloth().then(res => {
        this.$set(this.searchData._cloth, 'dataSource', res)
      })
    },
    // 成份下拉
    getCompositionSelect() {
      this.getComposition().then(res => {
        this.$set(this.searchData._ingredient, 'dataSource', res)
      })
    },
    ...
    getYarnSelect(type) {
      ...
      this.getYarn(GenericType).then(res => {
        this.$set(this.searchData._yarn, 'dataSource', res)
      })
    },
    // 纱类、丝类下拉 type: yarn纱类 silk丝类 combine组合
    getYarnTypeSelect(type) {
      this.getYarnType(type).then(res => {
        this.$set(this.searchData._yarnCategory, 'dataSource', res)
      })
    },
    // 印花工艺下拉
    getPrintCraftSelect(type) {
      this.getPrintCraft().then(res => {
        this.$set(this.searchData._printCraft, 'dataSource', res)
      })
    },
    selectionChange(val) {
      console.log(val)
