  ...
  data() {
    ...
    const baseData = {
      ...
      _marketType: {
        ...
        visibleChange: (value) => {
          if (value) {
            this.getOptionConfigSelect({ pageName: '大货订单', fieldName: '市场类型' }, 'marketType')
          }
        },
        dataSource: [],
        renderContent: this.renderContent
    ...
    return {
      ...
      formOptions: {
        ...
        syncDataHandle: (syncData) => {
          console.log('syncDatakmn', syncData)
          const dataArray = ['code', 'exchangeRate', 'orderType', 'brand', 'customer', 'salesTeam', 'seller', 'customerService', 'marketType', 'sellYear', 'sellQuarter', 'currency', 'taxRate', 'pricingTerm', 'paymentTerm']
          Object.keys(syncData).forEach(item => {
            if (!dataArray.includes(item)) {
              delete syncData[item]
            }
          })
          console.log('syncDat8585a', syncData)
    ...
    data: {
      handler(v) {
        if (v && Object.keys(v).length) {
          ...
          if (v.isFromCrm === '1') {
            console.log(this.marketTypeList)
            const marketType = this.marketTypeList.filter(item => { return item.label === '国内' })
            console.log(marketType)
            v.marketType = marketType.length ? marketType[0] : ''
            // v.taxRate = 13
          }
          const sup = ['brand', 'customer', 'seller', 'sellYear', 'sellQuarter', 'salesTeam', 'orderType', 'customerService', 'marketType', 'currency', 'pricingTerm', 'paymentTerm']
          sup.forEach(item => {
          ...
          this.$nextTick(() => {
            ...
            // 是结算 并且有币别
            if (v.currency && v.currency.label) {
    ...
    isDomestic(value) {
      if (value) {
        ...
        if (value.label === '国内') {
          ...
          // })
          // this.taxRate = 13
          // this.$set(this.baseData._currency, 'dataSource', currency && currency.length ? currency : null)
