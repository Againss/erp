...
export default {
      ...
      async handler(v) {
        ...
        if (v && Object.keys(v).length) {
          ...
          if (v.sampleAnalysisResultId) {
            ...
            this.getSampleAnalysisResultId(v.sampleAnalysisResultId)
  ...
  methods: {
    ...
    getSampleAnalysisResultId(id) {
      if (this.sampleAnalysisResultOption.length) {
        const tempArray = this.sampleAnalysisResultOption.filter(item => item.value === id)
        this.sampleAnalysisResultIdName = tempArray.length ? tempArray[0].label : ''
      }
