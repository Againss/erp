 ...
 * @LastEditors: OBKoro1
 * @LastEditTime: 2021-04-21 15:08:36
 * @FilePath: e:\erpmu\erp2-sf-2.0.3\src\views\customer-order\goods-orders\detail\compoments\qualityRequirement.vue
-->
<template>
  ...
  data() {
    ...
    const baseData = {
      ...
      _lstLightSource: {
        ...
        clearable: true,
        disabled: true,
        rules: [{ required: true, message: '1st对色光源不能为空', trigger: 'change' }],
        change: (value) => {},
        visibleChange: (value, val) => {
          if (value) {
            this.getFirstLights()
    ...
    const Inspection = {
      ...
      _checkClothReportJson: {
        ...
        disabled: true,
        multiple: true,
        rules: [{ required: true, message: '验布报告不能为空', trigger: 'change' }],
        dataSource: [
          {
            label: '系统数据',
            value: '1'
