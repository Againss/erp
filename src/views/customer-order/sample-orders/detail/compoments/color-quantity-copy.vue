 ...
 * @LastEditors: OBKoro1
 * @LastEditTime: 2021-07-15 15:39:44
 * @FilePath: d:\yangban\src\views\customer-order\sample-orders\detail\compoments\color-quantity-copy.vue
-->
<template>
  <div class="color color-copyIndex">
    <!-- 表头文字 -->
...
export default {
  ...
  props: {
    ...
    printCraftChanged: { // 印花工艺改变 ，需要清空分录花号字段
      type: String
    },
    isCloth: {
      type: String
    ...
    const allOperatesData = [
      {
        ...
        handle: scope => {
          if (this.isShowoperate()) {
            ...
            const obj = {
              isSettlement: vom.isSettlement, // 是否结算
              istype: 'edit'
            }
  ...
  watch: {
    ...
    printCraftChanged: {
      handler(v) {
        this.ClearPrintCraftChanged()
        console.log('22印花工艺改变 ，需要清空分录花号字段')
      },
      deep: true
    },
    isCloth: {
      handler(v) {
        v && this.setColorInfoType()
      },
      deep: true
    },
  ...
  methods: {
    ...
    // 印花信息改变 需要清空花号
    ClearPrintCraftChanged() {
      setTimeout(() => {
        const dom = this.colorQuantityDeliverySummary
        dom && dom.colorInfo && dom.colorInfo.length && dom.colorInfo.forEach((ix) => {
          ix.flowerNo = '' // 花号
          ix.flowerInfoType = '' // se
        })
      }, 50)
    },
    // 布类的改变 需要清空颜色信息和印花信息
