 ...
 * @LastEditors: OBKoro1
 * @LastEditTime: 2021-07-27 17:40:17
 * @FilePath: d:\erp\src\views\customer-order\sample-orders\detail\common\all-pop-options.vue
-->
<template>
  <div class="pop allPop LittleRedStarColor">
    <cs-custom-pop ref="allPop" :options="allPopOptions" class="LittleRedStarColor_allpop" />
    <!--剪板要求弹窗-->
    <plate-cutting
    ...
    />
    <!--条纹间距弹窗-->
    <stripe ref="stripeSpacingPop" :rules="rulea" :pop-data-fun="popDataFun" :plate-index-from="plateIndexFrom" />
    <!--数量交期弹窗-->
    <quantity
    ...
    <choose-pop
      ...
      @savePop="savePop"
    />
    <codePop ref="codePop" :pop-get-data="popGetData" @okHandleClick="OkClick" />
  </div>
</template>
...
export default {
  ...
  data() {
    ...
    var BlurFactoryNum = (rule, value, callback) => {
      ...
      } else if (Number(this.visibleFromData.refQuotedPrice) > Number(value)) {
    ...
    const quantityDeliveryColumns = {
      ...
      _amountTax: {
        ...
        showOverflowTooltip: true,
        isShow: (scope) => {
          return this.deliFreshData && this.deliFreshData.isSettlement
        },
        formater: (scope) => {
    ...
    const allContent = {
      ...
      _flowerInfoType: {
        ...
        disabled: false,
        isShow: (scope) => {
          return this.getShow(this.FormDatasInfo, '颜色信息') && this.getIsPrintCraft
        },
      ...
      _flowerNo: {
        ...
        clearable: true,
        isShow: (scope) => {
          return this.allPopOptions.formDatas.flowerInfoType && this.getShow(this.FormDatasInfo, '颜色信息') && this.getIsPrintCraft
        },
        clear: (scope, form, formDatas, setFormDatas) => {
      ...
      _quotationInformation: {
        ...
        text: '报价信息',
        isShow: (scope) => {
          return this.deliFreshData.isSettlement
        },
  ...
  methods: {
    ...
    addAllInfoCopy(data) {
      ...
      if (data.istype === 'edit') {
        ...
        // 颜色信息
        this.$set(this.allContent._colorInfoType, 'dataSource', this.setArrayInfo(dely.colorInfoType))
        // 印花信息
        this.$set(this.allContent._flowerInfoType, 'dataSource', this.setArrayInfo(dely.flowerInfoType))
        if (dely.styleNumber) {
          // 配色要求
        ...
        if (this.stripeSpacingData && this.stripeSpacingData.length) { // 动态添加校验 stripeSpacingPopOptions()
    ...
    priceTaxChange(data) {
      ...
      if (data && this.taxRate) {
        ...
        this.allPopOptions.formDatas = { ...this.visibleFromData, priceTax: data }
        this.allPopOptions.formDatas = { ...this.visibleFromData, price: price }
    ...
    // 报价单号
    OkClick(val) {
      console.log('报价单号2222222', val)
      this.allPopOptions.formDatas = { ...this.visibleFromData, quotationNo: val.quotationId, refQuotedPrice: this.$utils.getFloat(val.salePrice * this.exchangeRateArraya[0].label, 2) + '' }
