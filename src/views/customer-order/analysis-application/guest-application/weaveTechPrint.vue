 ...
 * @LastEditors: OBKoro1
 * @LastEditTime: 2021-01-12 17:37:49
-->
<template>
...
export default {
  ...
  methods: {
    ...
    pintBack() {
      if (this.$route.query.id) {
        const sk = this.$route.query.id
        this.$router.push({
          name: 'guestApplicationDetail',
          params: { id: sk, edit: 'show' }
        })
      }
    },
    // 获取分析单详情
    async getDetailFun(id) {
      ...
      if (sm && sm.machineTechnology && sm.machineTechnology.stitchMap) { // 排针图
        const stitchMap = sm.machineTechnology.stitchMap && JSON.parse(sm.machineTechnology.stitchMap)
    ...
    chemicalFibersTwo(item) {
      ...
      const som = item.composeWay ? (Number(item.composeWay) === 1 ? '' : 'x' + item.composeWay) : ''// 组合方式
      const element = that.ticone(item, item.elementInfo, 'label') // 成份
      ...
      return fineness + som + element + wireCategory + brand + gridDegree + lightDegree
    },
    ...
    getMachineYarn(data1, data2) {
      ...
      // return data2 || ''
      if (!data2) return ''
      if (data2.substring(0, 1) === '{') {
        const obj = data2 && JSON.parse(data2)
        let str1 = ''
        let str2 = ''
        const that = this
        obj.tableForm && obj.tableForm.forEach((ele, im) => {
