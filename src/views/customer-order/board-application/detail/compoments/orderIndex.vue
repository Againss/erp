...
export default {
  ...
  data() {
    return {
      ...
      maxDetailSeq: 0,
      baseInfo: {}, // 基础信息数据
      cutSampleRequirements: [], // 颜色数量交期数据
      status: '0' // 订单状态
    }
  },
  computed: {
  ...
  methods: {
    setOrderData(obj, index = 0) { // 设置订单要求页签下所有模块数据
      ...
      if (obj.hasOwnProperty('baseInfo')) {
        this.baseInfo = this.$utils.deepClone(obj.baseInfo)// 基础信息数
      }
      if (obj.materialInfo && obj.materialInfo.length) {
        // this.resetSeqCode(obj.materialInfo)
        this.status = obj.baseInfo.status
        this.cutSampleRequirements = this.$utils.deepClone(obj.cutSampleRequirements)
    ...
    syncMoudleDataHandle(key, data) {
      this.$nextTick(() => {
        ...
        if (key === 'baseInfo') { // 同步寄件要求模块数据
          const pm = { ...com.baseInfo, ...data }
          this.freshData = { ...this.freshData, baseInfo: pm }
        } else if (key === 'cutSampleRequirements') { // 同步剪样要求模块数据
          this.freshData = { ...this.freshData, cutSampleRequirements: data || [] }
        }
      })
    },
