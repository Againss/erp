 ...
 * @LastEditors: OBKoro1
 * @LastEditTime: 2021-07-27 15:26:54
 * @FilePath: d:\erp\src\views\customer-order\public\components\popupInformation.vue
-->
<template>
  <div class="come_up">
    <div class="come_san"></div>
    <div class="come_div">
       <div class="come_div_p">选择列表字段</div>
       <div class="come_some" >
        <el-checkbox style="color: #151222;" v-model="checkedUp"  :checked="checkedUp" @change="checkedUpClick">全选</el-checkbox>
       </div>
       <!-- list -->
          ...
          <div v-for="(item, index) in infoData.length" :key="index" class="come_single_yu">
            ...
            <div class="single_dv">
              <el-checkbox-group v-model="checkedCities" @change="handleCheckedCitiesChange">
                <el-checkbox v-for="city in infoData[index]" :label="city" :key="city" >{{city}}</el-checkbox>
              </el-checkbox-group>
            </div>
          </div>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      infoData:[
        ['序号','订单号','订单类型','需求来源','需求单号','品牌商','客户', '制衣厂', '销售团队', '销售季节','销售年度'], //基本信息
  ...
  methods: {
    ...
    // 转化数组
    tempArrClick(){
      let sk=this.infoData.join(',').split(',');
      this.tempString=sk
      return sk
    },
    personArray(){ //数组合并
      ...
      let sm =[...this.listArrays,...this.operArrays]
      let tu=new Set(sm)
    ...
    okHandle(){
     if(this.checkedCities.length){
      let obj={data:this.checkedCities,type:false}
