...
<template>
  <div id="PrintRequisition" class="list PrintRequisition">
    <div class="search">
      <cs-custom-form
        ref="searchForm"
    ...
    <div class="bk" style="position: relative">
      <div v-if="activeName === 'W' && $permission(['pd:pdPrintAllotDetail:submit'])" class="button spe">
        <el-button
          size="small"
          type="primary"
          @click="saveBtn"
  ...
  data() {
    ...
    const barConfig = {
      _change: {
        ...
        isShow: () => {
          return (this.getStatusInfo === 'W' || this.getStatusInfo === 'R') && !this.detailData._detailBranch.componentsOptions.isEdit && this.$permission(['pd:pdPrintAllotDetail:submit'])
        },
        submit: () => {
      ...
      _back: {
        ...
        buttonClass: 'sub-main',
        isShow: () => {
          return this.getStatusInfo === 'S' && !this.detailData._detailBranch.componentsOptions.isEdit && this.$permission(['pd:pdPrintAllotDetail:withdraw'])
        },
        submit: () => {
          this.back()
        }
      ...
      _cancel: {
        ...
        buttonClass: 'sub-main',
        isShow: () => {
          return (this.getStatusInfo === 'S' || this.getStatusInfo === 'R') && !this.detailData._detailBranch.componentsOptions.isEdit && this.$permission(['pd:pdPrintAllotDetail:cancel'])
        },
        submit: () => {
  ...
  methods: {
    ...
    async getInfo(params) {
      ...
      if (res.code === 200) {
        ...
        options.details = res.data.pdPrintAllotDetailInfoRespList || []
        // this.versionOptions =
        //   (res.data.versions &&
    ...
    async back() {
      ...
      if (res.code === 200) {
        ...
        this.$store.dispatch('app/fetchParamsLog', {
          opratorType: 'UPDATE',
          api: '/processingMgr/pdPrintAllotDetail/recall',
          description: {
            beforeText: `在'加工管理-印花-色布调拨通知单中修改通知单号 ${this.uuid} 的状态`,
            afterText: { status: 'R' },
            beforeCode: { status: 'S' }
          },
          appId: 'processingMgr'
    ...
    async submitForm() {
      ...
      if (res.code === 200) {
        ...
        this.$store.dispatch('app/fetchParamsLog', {
          opratorType: 'UPDATE',
          api: '/processingMgr/pdPrintAllotDetail/submit',
          description: {
            beforeText: `在'加工管理-印花-色布调拨通知单中修改通知单号 ${this.uuid} 的状态`,
            afterText: { status: 'S' },
            beforeCode: { status: 'W' }
    ...
    async cancel() {
      ...
      if (res.code === 200) {
        ...
        this.$store.dispatch('app/fetchParamsLog', {
          opratorType: 'UPDATE',
          api: '/processingMgr/pdPrintAllotDetail/cancel',
          description: {
            beforeText: `在'加工管理-印花-色布调拨通知单中修改通知单号 ${this.uuid} 的状态`,
            afterText: { status: 'C' },
            beforeCode: { status: 'R' }
    ...
    async allotSubmit() {
      const res = await api.allotConfirm({
        uuid: this.uuid,
        pdPrintAllotDetailUpdateSubsReqList: this.updateForm
      })
      this.$message({
...
#PrintRequisition.PrintRequisition {
  ...
  .print-requisition-detail-form {
    ...
    background: #fff;
    width: 100%;
    > div:not(.PrintRequisition-DetailBranch) {
      padding-left: 10px;
