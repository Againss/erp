 ...
 * @LastEditors: anthony
 * @LastEditTime: 2021-07-19 14:30:44
 * @FilePath: e:\erpmu\erp2-processing-v2.0.0\src\views\processing-manage\yarn-dyeing\requisition\index.vue
-->
<template>
  <div id="WeavingOrder_requisitionx" class="list">
    ...
    <div class="bk" style="position: relative;">
      <div class="tabs">
        ...
        <div class="main">
          <transitionWarp height="256px" width="288px" class-name="yarn-dyeing-requisition-transition" :is-show="showMore" @isShow="(val) => {showMore = val;}">
            <!-- 列表   -->
            <template v-slot:left>
              <div class="leftBox">
                <div v-if="getTaskNo" class="close-right" @click="showMore = !showMore">
                  <i :class="{'el-icon-arrow-right': showMore,'el-icon-arrow-left': !showMore}" />
                </div>
                <div
                  class="detail"
                  :class="{ 'page-table': !showMore }"
                  :style="{
                    height: !showMore ? '100%' : 'calc((100vh - 256px}))', 'padding-left':'10px'
                  }"
                >
                  <cs-custom-table
                    :key="activeName"
            ...
            <template v-slot:right>
              <div class="rightBox">
                ...
                <div class="clearfix" style="position: relative;background: #fff;">
                  ...
                  <div v-if="getTaskNo&&!GlobalSwitch" class="button spe"><buttonBar :config="barConfig" :wipe-class="true" /> </div>
                </div>
                <!-- 下拉信息 -->
                <div class="page-detail">
                  <details-com :active-index="activeName" :is-edit="isEdit" :data="orderIndexData" :sync-moudle-data-handle="syncMoudleDataHandle" />
                </div>
              </div>
            </template>
          </transitionWarp>
...
<script>
import { list, orderStatusNum, orderInfo, orderSubmit, orderWithdrawt, orderCancel } from './api/index'
...
export default {
  ...
  data() {
    const searchData = {
      ...
      _allotOutAddr: {
        ...
        placeholder: '请输入调出地址',
        clear: (form, formDatas, setFormDatas, scope) => {
          this.searchFormDatas.allotOutAddr = ''
        },
        clearable: true
      },
      _sourceNoAddr: {
        ...
        placeholder: '请输入调入地址',
        clear: (form, formDatas, setFormDatas, scope) => {
          this.searchFormDatas.sourceNoAddr = ''
        },
        clearable: true
      },
      _createdTime: {
    ...
    const columns = {
      ...
      _sourceNoAddr: {
        ...
        minWidth: '150',
        showOverflowTooltip: true,
        formater: scope => scope.row.sourceNoAddr || '-'
      },
      _applyCode: {// pm
        prop: 'applyCode',
        label: '调拨数量(KG)',
    ...
    const tabs = [
      ...
      { label: '待提交', value: 1, count: 0 },
      { label: '已提交', value: 2, count: 0 },
      { label: '已回撤', value: 3, count: 0 },
      ...
      componentOptions: {
        config: {
          _m1: {
            ...
            num: 0,
            formater: (val) => {
              return this.totalNum && Object.keys(this.totalNum).length ? this.totalNum.amount : '0'
            }
          }
        }
      }
    ...
    const barConfig = {
      _orderChange: {
        ...
        isShow: () => { return ['W', 'R'].includes(this.getOrdestatus) && this.$permission(['pd:pdDyeingAllotNotify:modify']) },
        type: '',
        submit: () => { this.isEdit = true; this.isSubmit = true; this.GlobalSwitch = true }
      },
      _Submit: {
        text: '提交',
        buttonClass: 'sub-main',
        isShow: () => { return this.getOrdestatus === 'W' && !this.isSubmit && this.$permission(['pd:pdDyeingAllotNotify:submit']) },
        type: '',
        submit: () => {
      ...
      _withdraw: {
        text: '撤回',
        buttonClass: 'sub-main',
        isShow: () => { return this.getOrdestatus === 'S' && this.$permission(['pd:pdDyeingAllotNotify:withdraw']) },
        type: '',
        submit: () => {
      ...
      _cancel: {
        text: '取消',
        buttonClass: 'sub-main',
        isShow: () => { return ['S', 'R'].includes(this.getOrdestatus) && !this.isSubmit && this.$permission(['pd:pdDyeingAllotNotify:cancel']) },
        type: '',
        submit: () => {
  ...
  methods: {
    ...
    async list(data = {}, fresh) {
      ...
      this.tabs[0].count = comIndex.total || 0
      this.tabs[1].count = comIndex.W || 0
      this.tabs[2].count = comIndex.S || 0
      this.tabs[3].count = comIndex.R || 0
      this.tabs[4].count = comIndex.C || 0
      if (this.$route.query && this.$route.query.noticeNo) {
        this.$router.replace({})
      }
    },
    syncMoudleDataHandle(key, data) { // 同步右侧各个模块的全量表单数据
      console.log('sdfsf', key, data)
      this.$nextTick(() => {
        if (key === 'productData') { // 同步客样分析编号模块数据
          this.freshData = { ...this.freshData, ...data }
        } else if (key === 'materialData') {
          if (this.freshData && this.freshData.details && this.freshData.details.length) {
    ...
    rowClickIndex(row) {
      this.showMore = true
      this.indexRow = row
      console.log('ommm', this.indexRow)
      this.getDetailData()
    },
    okCallback() {
      this.list({}, true)
    },
    async SubmitClick(parem = {}) { // 提交接口
