 ...
 * @LastEditors: anthony
 * @LastEditTime: 2021-07-19 14:30:37
 * @FilePath: e:\erpmu\erp2-processing-v2.0.0\src\views\processing-manage\yarn-dyeing\order\index.vue
-->
...
export default {
  ...
  data() {
    ...
    const barConfig = {
      ...
      _print: {
        ...
        submit: () => {
          this.$router.push(
            '/processing/yarn-dyeing/order/print/' + this.indexRow.orderId
          )
        }
      }
    }
  ...
  methods: {
    ...
    async list(data = {}, fresh) {
      ...
      // CANCEL("C", "取消"),
      // FINISH("F", "完工"),
      this.tabs[0].count = comIndex.total
      this.tabs[1].count = comIndex.W
      this.tabs[2].count = comIndex.S
      this.tabs[3].count = comIndex.R
      this.tabs[4].count = comIndex.F
      this.tabs[5].count = comIndex.C
      if (this.$route.query && this.$route.query.orderId) {
