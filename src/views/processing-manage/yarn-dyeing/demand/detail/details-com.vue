...
export default {
        ...
        syncDataHandle: (syncData) => {
          ...
          if (this.multipleSelection && this.multipleSelection.length) {
            ...
            IndexData.forEach((im) => {
              this.multipleSelection.forEach((ic) => {
      ...
      handler(v) {
        if (v && Object.keys(v).length) {
          ...
          comData.forEach((item) => {
            ...
            if (item.notQty === 0 || item.notQty === '0') {
              item.edit = false
            } else {
            ...
            if (this.isEdit) {
              ...
              item.dyeingDelivery =
                (!item.dyeingDelivery && item.planDelivery) ||
                item.dyeingDelivery
            }
  ...
  methods: {
    isSing(scope) {
      if (
        ['dyeingDelivery', 'factoryNum'].includes(scope.column.property) &&
        this.isEdit
      ) {
        return 'isSing'
      }
      ...
      if (!this.multipleSelection.length) {
        ...
        this.$nextTick(() => {
          this.$refs.materialData &&
            this.$refs.materialData.form.clearValidate()
        })
      }
    },
    syncPop(key, data) {
          ...
          comData.forEach((ix) => {
            this.multipleSelection.forEach((it) => {
              if (ix.iscodeIndex === it.iscodeIndex) {
                setTimeout(() => {
                  this.table.toggleRowSelection(ix, true)
                }, 0)
              }
            })
          })
