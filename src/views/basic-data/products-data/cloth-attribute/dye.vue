...
export default {
    ...
    let popOperatesDataSource = [
      {
        ...
        handle: scope => {
          this.$confirm('确认审核当前数据吗？', '提示', {
            ...
            .then(() => {
              this.dyeWayApprove(scope.row.id)
            })
            .catch(() => {
              this.$message({
                type: 'info',
      ...
      },
      {
        ...
        handle: scope => {
          this.$confirm('确认删除当前数据吗？', '提示', {
            ...
            .then(() => {
              this.dyeWayDelete(scope.row.id)
            })
            .catch(() => {
              this.$message({
                type: 'info',
  ...
  methods: {
    ...
    async getListData(data = {}) {
      const res = await sysDataGroupPage({ ...data, mode: 'D' })
    ...
    // 新增染方式分类
    async dyeWayAdd(data = {}) {
      const res = await sysDataGroupAdd({ ...data, mode: 'D' })
      if (res.code !== 200) {
        return false
      }
    ...
    // 编辑染方式分类
    async dyeWayEdit(data = {}) {
      const res = await sysDataGroupModify({ ...data, mode: 'D' })
      if (!res || res.code !== 200) {
        return false
      }
      this.$set(this.popOptions, 'visible', false)
