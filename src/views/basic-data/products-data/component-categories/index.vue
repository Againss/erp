 ...
 * @LastEditTime: 2020-10-16 14:30:50
 * @LastEditors: shujing
 **  成分大类页面
 -->
<template>
  <div class="categoriesPage">
    ...
    <div class="button">
      <el-button v-permission="['basic:categories:store']" type="primary" size="small" icon="el-icon-plus" @click="addList">新增</el-button>
    </div>
    <!-- table表格 -->
...
<script>
import { categoriesList, categoriesAdd, categoriesEdit, categoriesUpdate, categoriesDelete, categoriesApprove, categoriesEnabled } from './api/index.js'
...
export default {
  ...
  data() {
    ...
    const columns = [
      ...
      },
      {
        ...
        componentsOptions: {
          permitTag: 'basic:categories:enabled',
          changeStatus: (scope) => {
    ...
    let popOperatesDataSource = [
      {
        label: '审核',
        permitTag: ['basic:categories:approve'],
        isShow: (scope) => {
      ...
      {
        label: '编辑',
        permitTag: ['basic:categories:update'],
        isShow: true,
        handle: scope => {
      ...
      },
      {
        label: '删除',
        style: { 'color': '#FE4949' },
        permitTag: ['basic:categories:destroy'],
        isShow: (scope) => {
          return scope.row.status.code === 'B'
        },
        handle: scope => {
  ...
  methods: {
    ...
    // 新增的确定事件
    async addData(data = {}) {
      const res = await categoriesAdd(data)
      if (res.code !== 200) {
        return false
      }
    ...
    async getRulePage(data = {}) {
      const res = await categoriesList({ ...this.searchFormDatas, ...data })
    ...
    async editList(data = {}, code) {
      const res = await categoriesEdit(data)
      if (!res || res.code !== 200) {
        return false
      }
    ...
    async addUpdate(data = {}) {
      const res = await categoriesUpdate(data)
      if (!res || res.code !== 200) {
        return false
      }
      this.$set(this.popOptions, 'visible', false)
    ...
    async approveData(id) {
      const res = await categoriesApprove({ id: id })
      if (res.code !== 200) {
        return false
      }
      this.$message({
    ...
    // 删除
    async deleteData(id) {
      const res = await categoriesDelete({ id: id })
      if (res.code !== 200) {
        return false
      }
      this.$message({
    ...
    async changeEnable(data = {}, scope) {
      const res = await categoriesEnabled(data).catch(() => {
