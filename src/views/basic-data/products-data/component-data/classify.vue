...
<template>
  <div class="classifyPage">
    ...
    <!-- <div class="button">
      <el-button v-permission="['basic:IngredientClassifications:store']" type="primary" size="small" icon="el-icon-plus" @click="addList">新增</el-button>
    </div> -->
    <!-- table表格 -->
...
<script>
import { classifyList, classifyAdd, classifyDetail, classifyUpdate, classifyDelete, classifyApprove, classifyEnabled } from './api/index.js'
...
export default {
  ...
  data() {
    ...
    const columns = [
      ...
      },
      {
        ...
        componentsOptions: {
          permitTag: 'basic:IngredientClassifications:enabled',
          changeStatus: (scope) => {
    ...
    const commonContent = [
      ...
      },
      {
        ...
        trim: ruleTrim,
        clearable: true,
        rules: [commonBlurReg, { validator: checkData, message: '简称必须为一个大写字母', trigger: ['change'] }],
    ...
    let popOperatesDataSource = [
      {
        label: '审核',
        permitTag: ['basic:IngredientClassifications:approve'],
        isShow: (scope) => {
      ...
      {
        label: '编辑',
        permitTag: ['basic:IngredientClassifications:update'],
        isShow: true,
        handle: scope => {
      ...
      },
      {
        label: '删除',
        style: { 'color': '#FE4949' },
        permitTag: ['basic:IngredientClassifications:destroy'],
        isShow: (scope) => {
          return scope.row.status.code === 'B'
        },
        handle: scope => {
  ...
  methods: {
    ...
    // 新增的确定事件
    async addData(data = {}) {
      const res = await classifyAdd(data)
      if (res.code !== 200) {
        return false
      }
    ...
    async getRulePage(data = {}) {
      const res = await classifyList({ ...this.searchFormDatas, ...data })
    ...
    async editList(data = {}, code) {
      const res = await classifyDetail(data)
      if (!res || res.code !== 200) {
        return false
      }
    ...
    async addUpdate(data = {}) {
      const res = await classifyUpdate(data)
      if (!res || res.code !== 200) {
        return false
      }
      this.$set(this.popOptions, 'visible', false)
    ...
    async approveData(id) {
      const res = await classifyApprove({ id: id })
      if (res.code !== 200) {
        return false
      }
      this.$message({
    ...
    // 删除
    async deleteData(id) {
      const res = await classifyDelete({ id: id })
      if (res.code !== 200) {
        return false
      }
      this.$message({
    ...
    async changeEnable(data = {}, scope) {
      const res = await classifyEnabled(data).catch(() => {
