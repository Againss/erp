...
<template>
  <div class="currencyPage">
    ...
    <!-- 新增菜单按钮 -->
    <div class="button">
      <el-button v-permission="['basic:currencies:store']" type="primary" size="small" icon="el-icon-plus" @click="addList">新增</el-button>
    </div>
    <!-- table表格 -->
...
<script>
import { currencySearch, currencyDetail, currencyAdd, currencyUpdate, currencyDelete, currencyApprove, currencyEnabled } from './api/index.js'
...
export default {
  ...
  data() {
    ...
    const columns = [
      ...
      },
      {
        ...
        componentsOptions: {
          permitTag: 'basic:currencies:enabled',
          changeStatus: (scope) => {
    ...
    let popOperatesDataSource = [
      {
        label: '审核',
        permitTag: ['basic:currencies:approve'],
        isShow: (scope) => {
      ...
      {
        label: '编辑',
        permitTag: ['basic:currencies:update'],
        isShow: true,
        handle: scope => {
      ...
      },
      {
        label: '删除',
        style: { 'color': '#FE4949' },
        permitTag: ['basic:currencies:destroy'],
        isShow: (scope) => {
          return scope.row.status.code === 'B'
        },
        handle: scope => {
  ...
  methods: {
    ...
    // 新增的确定事件
    async addData(data = {}) {
      const res = await currencyAdd(data)
      if (res.code !== 200) {
        return false
      }
    ...
    addOrEdit(params) {
      ...
      if (this.type === 'add') {
        const obj = {}
        const pamraskey = ['name', 'nameEn', 'unit', 'code', 'remark']
        pamraskey.forEach(v => {
          obj[v] = params[v]
        })
        this.addData(obj)
        ...
        const obj = {}
        const pamraskey = ['id', 'name', 'nameEn', 'unit', 'code', 'remark']
        pamraskey.forEach(v => {
          obj[v] = params[v]
        })
        this.addUpdate(obj)
    ...
    async getRulePage(data = {}) {
      const res = await currencySearch({ ...this.searchFormDatas, ...data })
    ...
    async editList(data = {}, code) {
      const res = await currencyDetail(data)
      if (!res || res.code !== 200) {
        return false
      }
    ...
    async addUpdate(data = {}) {
      const res = await currencyUpdate(data)
      if (!res || res.code !== 200) {
        return false
      }
      this.$set(this.popOptions, 'visible', false)
    ...
    async approveData(id) {
      const res = await currencyApprove({ id: id })
      if (res.code !== 200) {
        return false
      }
      this.$message({
    ...
    // 删除
    async deleteData(id) {
      const res = await currencyDelete({ id: id })
      if (res.code !== 200) {
        return false
      }
      this.$message({
    ...
    async changeEnable(data = {}, scope) {
      const res = await currencyEnabled(data)
      if (res.code === 200) {
        this.$message({
