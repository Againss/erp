 ...
 * @LastEditTime: 2021-04-30 13:46:14
 * @LastEditors: admin
 **  价格条款页面
 -->
<template>
  <div class="pricePage">
    ...
    <div class="button">
      <el-button v-permission="['basic:priceTerms:store']" icon="el-icon-plus" size="small" type="primary" @click="addList">新增</el-button>
    </div>
    <!-- table表格 -->
...
<script>
import { pricePage, priceAdd, priceDetail, priceUpdate, priceDelete, priceApprove, priceEnabled } from './api/index.js'
...
export default {
  ...
  data() {
    ...
    const columns = [
      ...
      },
      {
        ...
        componentsOptions: {
          permitTag: 'basic:priceTerms:enabled',
          changeStatus: (scope) => {
    ...
    let popOperatesDataSource = [
      {
        label: '审核',
        permitTag: ['basic:priceTerms:approve'],
        isShow: (scope) => {
      ...
      {
        label: '编辑',
        permitTag: ['basic:priceTerms:update'],
        isShow: true,
        handle: scope => {
      ...
      },
      {
        label: '删除',
        style: { 'color': '#FE4949' },
        permitTag: ['basic:priceTerms:destroy'],
        isShow: (scope) => {
          return scope.row.status.code === 'B'
        },
        handle: scope => {
  ...
  methods: {
    ...
    async getRulePage(data = {}) {
      const res = await pricePage({ ...this.searchFormDatas, ...data })
    ...
    async editList(data = {}, code) {
      const res = await priceDetail(data)
      if (!res || res.code !== 200) {
        return false
      }
    ...
    async addUpdate(data = {}) {
      const res = await priceUpdate(data)
      if (!res || res.code !== 200) {
        return false
      }
      this.$set(this.popOptions, 'visible', false)
    ...
    async approveData(id) {
      const res = await priceApprove({ id: id })
      if (res.code !== 200) {
        return false
      }
      this.$message({
    ...
    // 删除
    async deleteData(id) {
      const res = await priceDelete({ id: id })
      if (res.code !== 200) {
        return false
      }
      this.$message({
    ...
    async changeEnable(data = {}, scope) {
      const res = await priceEnabled(data)
      if (res.code === 200) {
        this.$message({
