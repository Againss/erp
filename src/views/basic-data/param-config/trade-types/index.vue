 ...
 * @LastEditors: shujing
 * @LastEditTime: 2020-11-16 13:57:04
-->

<template>
  <div class="tradetypes">
    <!-- 搜索表单 -->
    <div class="search">
    ...
    <div class="button">
      <el-button
        v-permission="['basic:currentUser:store']"
        type="primary"
        icon="el-icon-plus"
        size="small"
        @click="addRange"
  ...
  data() {
    ...
    const columns = [
      ...
      },
      {
        ...
        componentsOptions: {
          permitTag: 'basic:currentUser:enabled',
          changeStatus: (scope) => {
    ...
    let popOperatesList = [
      {
        label: '审核',
        permitTag: ['basic:currentUser:approve'],
        isShow: (scope) => {
      ...
      {
        label: '编辑',
        permitTag: ['basic:currentUser:update'],
        isShow: true,
        handle: scope => {
      ...
      },
      {
        label: '删除',
        style: { 'color': '#FE4949' },
        permitTag: ['basic:currentUser:destroy'],
        isShow: (scope) => {
          return scope.row.status.code === 'B'
        },
        handle: scope => {
