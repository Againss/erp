 ...
 * @Description: 色级
 * @Autor: zhengjin
 * @Date: 2021-06-18 09:33:09
 * @LastEditors: zhengjin
 * @LastEditTime: 2021-06-28 14:07:21
-->
...
<template>
  <div class="categoriesPage product-list-content">
    <div class="button">
      <el-button
        v-permission="['basic:colorGrade:store']"
        size="small"
        type="primary"
        icon="el-icon-plus"
        @click="addList"
...
export default {
  ...
  data() {
    ...
    const checklowerLimit = (rule, value, callback) => {
      ...
      if (this.upperLimitNew === '' || this.upperLimitNew === 0) {
        callback()
        return
      }
      if (!reg.test(value) || value - 0 > this.upperLimitNew - 0) {
        ...
        callback()
        this.ruleForm.clearValidate(['upperLimit'])
      }
    }
    const checkupperLimit = (rule, value, callback) => {
      ...
      if (!reg.test(value) || value - 0 < this.lowerLimitNew - 0) {
        ...
        callback()
        this.ruleForm.clearValidate(['lowerLimit'])
      }
    }
    // 表格表头的配置信息
    const columns = [
      ...
      },
      {
        ...
        componentsOptions: {
          permitTag: 'basic:colorGrade:enabled',
          changeStatus: (scope) => {
            this.changeEnable(
              {
                id: scope.row.id,
    ...
    let popOperatesDataSource = [
      {
        label: '审核',
        permitTag: ['basic:colorGrade:approve'],
        isShow: (scope) => {
      ...
      {
        label: '编辑',
        permitTag: ['basic:colorGrade:update'],
        isShow: true,
        handle: (scope) => {
      ...
      {
        label: '删除',
        permitTag: ['basic:colorGrade:destroy'],
        isShow: (scope) => {
          return scope.row.status.code === 'B'
        },
        handle: params => {
          this.$confirm('确认删除当前数据吗？', '提示', {
            ...
            this.fetchLog(
              this.getLogMessages('UPDATE', '/basis/api/colorGrade/destroy', 'basis'),
  ...
  methods: {
    ...
    async addData(data = {}) {
      ...
      this.fetchLog(
        this.getLogMessages('INSERT', '/basis/api/colorGrade/store', 'basis'),
    ...
    addOrEdit(params) {
      ...
      if (this.type === 'add') {
        const obj = {}
        const pamraskey = ['name', 'abbr', 'lowerLimit', 'upperLimit', 'remark']
        pamraskey.forEach((v) => {
          obj[v] = params[v]
        })
        this.addData(obj)
        ...
        // 走编辑接口
        const obj = {}
        const pamraskey = ['id', 'name', 'abbr', 'lowerLimit', 'upperLimit', 'remark']
        pamraskey.forEach((v) => {
          obj[v] = params[v]
        })
        this.addUpdate(obj)
    ...
    async getRulePage(data = {}) {
      ...
      res.data && res.data.map((item, index) => { item.index = index + 1 })
    ...
    async editList(data = {}, code) {
      ...
      this.lowerLimitNew = editData.lowerLimit
      this.upperLimitNew = editData.upperLimit
    ...
    async addUpdate(data = {}) {
      ...
      this.fetchLog(
        this.getLogMessages('UPDATE', '/basis/api/colorGrade/update', 'basis'),
        { id: data.id, name: data.name },
        JSON.stringify({
    ...
    async approveData(id, scope) {
      ...
      this.fetchLog(
        this.getLogMessages('UPDATE', '/basis/api/colorGrade/approve', 'basis'),
    ...
    changeEnable: debounce(
      async function(data = {}, scope) {
        ...
        if (res.code === 200) {
          this.fetchLog(
            this.getLogMessages('UPDATE', '/basis/api/colorGrade/enable', 'basis'),
