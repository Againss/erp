...
<template>
  <div class="requestsort">
    ...
    <!-- <div class="button button-position">
      <el-button
        v-permission="['basic:requiredClassifications:store']"
        type="primary"
        icon="el-icon-plus"
        size="small"
        @click="addRange"
...
export default {
  ...
  data() {
    ...
    const columns = [
      ...
      },
      {
        ...
        componentsOptions: {
          permitTag: 'basic:requiredClassifications:enabled',
          changeStatus: (scope) => {
    ...
    let popOperatesList = [
      {
        label: '审核',
        permitTag: ['basic:requiredClassifications:approve'],
        isShow: (scope) => {
      ...
      {
        label: '编辑',
        permitTag: ['basic:requiredClassifications:update'],
        isShow: true,
        handle: scope => {
      ...
      },
      {
        label: '删除',
        style: { 'color': '#FE4949' },
        permitTag: ['basic:requiredClassifications:destroy'],
        isShow: (scope) => {
          return scope.row.status.code === 'B'
        },
        handle: scope => {
  ...
  methods: {
    ...
    async EnableStatus(data = {}, scope) {
      const res = await sysDataGroupEnable(data).catch(() => {
