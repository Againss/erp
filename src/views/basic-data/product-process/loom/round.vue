...
<template>
  <div class="round">
    ...
    <!-- <div class="button">
      <el-button
        v-permission="['basic:loomCirculars:store']"
        type="primary"
        icon="el-icon-plus"
        size="small"
        @click="addRange"
...
<script>
import {
  sysDataGroupIndexRound,
  sysDataGroupEnableRound,
  sysDataGroupAddRound,
  sysDataGroupModifyRound,
  sysDataGroupDeleteRound,
  sysDataGroupExamRound,
  sysDataGroupInfoRound
} from './api/index.js'
...
export default {
  ...
  data() {
    ...
    const columns = [
      ...
      },
      {
        ...
        componentsOptions: {
          permitTag: 'basic:loomCirculars:enabled',
          changeStatus: (scope) => {
    ...
    let popOperatesList = [
      {
        label: '审核',
        permitTag: ['basic:loomCirculars:approve'],
        isShow: (scope) => {
      ...
      {
        label: '编辑',
        permitTag: ['basic:loomCirculars:update'],
        isShow: true,
        handle: scope => {
      ...
      },
      {
        label: '删除',
        style: { 'color': '#FE4949' },
        permitTag: ['basic:loomCirculars:destroy'],
        isShow: (scope) => {
          return scope.row.status.code === 'B'
        },
        handle: scope => {
  ...
  methods: {
    ...
    async EnableStatus(data = {}, scope) {
      const res = await sysDataGroupEnableRound(data)
      if (res.code === 200) {
        this.$message({
    ...
    addOrModifyGroup(params) {
      if (this.type === 'add') {
        const obj = {}
        const pamraskey = ['name', 'nameEn', 'typical', 'remark']
        pamraskey.forEach(v => {
          obj[v] = params[v]
        })
        this.addGroup(obj)
        ...
        const obj = {}
        const pamraskey = ['id', 'name', 'nameEn', 'typical', 'remark']
        pamraskey.forEach(v => {
          obj[v] = params[v]
        })
        this.editGroup(obj)
    ...
    // 新增保存
    async addGroup(data = {}) {
      const res = await sysDataGroupAddRound(data)
      if (res.code !== 200) {
        return false
      }
    ...
    async editGroup(data = {}) {
      const res = await sysDataGroupModifyRound(data)
      if (res.code !== 200) {
        return false
      }
    ...
    async deleteGroup(data = {}) {
      const res = await sysDataGroupDeleteRound(data)
      if (res.code !== 200) {
        return false
      }
    ...
    async examGroup(data = {}) {
      const res = await sysDataGroupExamRound(data)
      if (res.code !== 200) {
        return false
      }
    ...
    async getGroupInfo(data = {}, scope) {
      const res = await sysDataGroupInfoRound(data)
      if (res.code !== 200) {
        return false
      }
      const dataList = res.data || {}
      dataList.code = scope.row.code
      dataList.typical = dataList.typical.code
