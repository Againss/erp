 ...
 * @version:
 * @Author: shujing
 * @Date: 2020-09-29 09:28:19
 * @LastEditors: shujing
 * @LastEditTime: 2020-11-16 13:55:14
-->

<template>
  <div class="print-dyeing">

    <!-- 新增角色按钮 -->
    <div class="button">
      <el-button
        v-permission="['basic:printingDyeingType:store']"
        type="primary"
        icon="el-icon-plus"
        size="small"
        @click="addRange"
  ...
  data() {
    ...
    const columns = [
      ...
      },
      {
        ...
        componentsOptions: {
          permitTag: 'basic:printingDyeingType:enabled',
          changeStatus: (scope) => {
    ...
    let popOperatesList = [
      {
        label: '审核',
        permitTag: ['basic:printingDyeingType:approve'],
        isShow: (scope) => {
      ...
      {
        label: '编辑',
        permitTag: ['basic:printingDyeingType:update'],
        isShow: true,
        handle: scope => {
      ...
      },
      {
        label: '删除',
        style: { 'color': '#FE4949' },
        permitTag: ['basic:printingDyeingType:destroy'],
        isShow: (scope) => {
          return scope.row.status.code === 'B'
        },
        handle: scope => {
