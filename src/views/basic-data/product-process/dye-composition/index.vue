 ...
 * @LastEditors: shujing
 * @LastEditTime: 2020-11-16 13:57:33
-->

<template>
  <div class="dye-composition">
    <!-- 新增角色按钮 -->
    <div class="button">
      <el-button
        v-permission="['basic:dyeIngredient:store']"
        type="primary"
        icon="el-icon-plus"
        size="small"
        @click="addRange"
  ...
  data() {
    ...
    const columns = [
      ...
      },
      {
        ...
        componentsOptions: {
          permitTag: 'basic:dyeIngredient:enabled',
          changeStatus: (scope) => {
    ...
    let popOperatesList = [
      {
        label: '审核',
        permitTag: ['basic:dyeIngredient:approve'],
        isShow: (scope) => {
      ...
      {
        label: '编辑',
        permitTag: ['basic:dyeIngredient:update'],
        isShow: true,
        handle: scope => {
      ...
      },
      {
        label: '删除',
        style: { 'color': '#FE4949' },
        permitTag: ['basic:dyeIngredient:destroy'],
        isShow: (scope) => {
          return scope.row.status.code === 'B'
        },
        handle: scope => {
