 ...
 * @LastEditTime: 2021-03-12 17:24:55
 * @LastEditors: anthony
 **  工序流程页面
 -->
<template>
  <div class="processPage">
    ...
    <div class="button">
      <el-button
        v-permission="['basic:processFlow:store']"
        type="primary"
        size="small"
        icon="el-icon-plus"
        @click="addList"
...
<script>
import { processList, processAdd, processEdit, processUpdate, processDelete, processApprove, processEnabled } from './api/index.js'
...
export default {
  ...
  data() {
    ...
    const columns = [
      ...
      },
      {
        ...
        componentsOptions: {
          permitTag: 'basic:processFlow:enabled',
          changeStatus: (scope) => {
    ...
    let popOperatesDataSource = [
      {
        label: '审核',
        permitTag: ['basic:processFlow:approve'],
        isShow: (scope) => {
      ...
      {
        label: '编辑',
        permitTag: ['basic:processFlow:update'],
        isShow: true,
        handle: scope => {
      ...
      },
      {
        label: '删除',
        style: { 'color': '#FE4949' },
        permitTag: ['basic:processFlow:destroy'],
        isShow: (scope) => {
          return scope.row.status.code === 'B'
        },
        handle: scope => {
  ...
  methods: {
    ...
    // 新增的确定事件
    async addData(data = {}) {
      const res = await processAdd(data)
      if (res.code !== 200) {
        return false
      }
    ...
    async getRulePage(data = {}) {
      const res = await processList({ ...this.searchFormDatas, ...data })
    ...
    async editList(data = {}, code) {
      const res = await processEdit(data)
      if (!res || res.code !== 200) {
        return false
      }
    ...
    async addUpdate(data = {}) {
      const res = await processUpdate(data)
      if (!res || res.code !== 200) {
        return false
      }
      this.$set(this.popOptions, 'visible', false)
    ...
    async approveData(id) {
      const res = await processApprove({ id: id })
      if (res.code !== 200) {
        return false
      }
      this.$message({
    ...
    // 删除
    async deleteData(id) {
      const res = await processDelete({ id: id })
      if (res.code !== 200) {
        return false
      }
      this.$message({
    ...
    async changeEnable(data = {}, scope) {
      const res = await processEnabled(data)
      if (res.code === 200) {
        this.$message({
