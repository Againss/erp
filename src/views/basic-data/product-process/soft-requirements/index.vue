 ...
 * @LastEditTime: 2021-03-16 14:56:06
 * @LastEditors: anthony
 ** 制软要求页面
 -->
<template>
  <div class="processPage">
    ...
    <div class="button">
      <el-button
        v-permission="['basic:softMethod:store']"
        type="primary"
        size="small"
        icon="el-icon-plus"
        @click="addList"
  ...
  data() {
    ...
    const columns = [
      ...
      },
      {
        ...
        componentsOptions: {
          permitTag: 'basic:softMethod:enabled',
          changeStatus: (scope) => {
    ...
    let popOperatesDataSource = [
      {
        label: '审核',
        permitTag: ['basic:softMethod:approve'],
        isShow: (scope) => {
      ...
      {
        label: '编辑',
        permitTag: ['basic:softMethod:update'],
        isShow: true,
        handle: scope => {
      ...
      },
      {
        label: '删除',
        style: { 'color': '#FE4949' },
        permitTag: ['basic:softMethod:destroy'],
        isShow: (scope) => {
          return scope.row.status.code === 'B'
        },
        handle: scope => {
