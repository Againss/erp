...
<template>
  <div class="singleDouble">
    <div class="button">
      <el-button v-permission="['basic:dyeType:store']" size="small" icon="el-icon-plus" type="primary" @click="addSingleDouble">新增</el-button>
    </div>
...
import { sysDataGroupPage, sysDataGroupAdd, sysDataGroupModify, sysDataGroupApprove, sysDataGroupDelete, sysDataGroupEnabled, sysDataGroupDetail } from './api/index.js'
export default {
  data() {
    // 公用验证正则
    ...
    const columns = [
      ...
      },
      {
        ...
        label: '单双染',
        formater: (scope) => {
          return scope.row.singleDoubleDyeObj && scope.row.singleDoubleDyeObj.name
        }
      },
      {
        prop: 'remark',
      ...
      },
      {
        ...
        componentsOptions: {
          permitTag: 'basic:dyeType:enabled',
          changeStatus: (scope) => {
    ...
    let popOperatesDataSource = [
      {
        label: '审核',
        permitTag: ['basic:dyeType:approve'],
        isShow: (scope) => {
        ...
        handle: scope => {
          this.$confirm('确认审核当前数据吗？', '提示', {
            ...
            .then(() => {
              this.singleDoubleApprove(scope.row.id)
            })
            .catch(() => {
              this.$message({
                type: 'info',
      ...
      {
        label: '编辑',
        permitTag: ['basic:dyeType:update'],
        isShow: true,
        handle: scope => {
      ...
      },
      {
        label: '删除',
        style: { 'color': '#FE4949' },
        permitTag: ['basic:dyeType:destroy'],
        isShow: (scope) => {
          return scope.row.status.code === 'B'
        },
        handle: scope => {
          this.$confirm('确认删除当前数据吗？', '提示', {
            ...
            .then(() => {
              this.singleDoubleDelete(scope.row.id)
            })
            .catch(() => {
              this.$message({
                type: 'info',
    ...
    const popOptions = {
      ...
      okText: '保存',
      ok: params => {
        this.singleDoubleOption(params)
      },
  ...
  methods: {
    ...
    addSingleDouble() {
      ...
      this.$set(this.content[3], 'disabled', false)
      this.$set(this.content[4], 'disabled', false)
      this.PopDialogHandle('新增染色类型')
      this.$set(this.popOptions, 'content', this.content)
      this.$set(this.popOptions, 'formDatas', {})
    },
    // 新增、编辑单双染
    singleDoubleOption(params) {
      if (this.type === 'add') {
        const obj = {}
        const pamraskey = ['name', 'nameEn', 'abbr', 'singleDoubleDye', 'remark']
        pamraskey.forEach(v => {
          obj[v] = params[v]
        })
        this.singleDoubleAdd(obj)
      } else if (this.type === 'edit') {
        const obj = {}
        const pamraskey = ['id', 'name', 'nameEn', 'abbr', 'singleDoubleDye', 'remark']
        pamraskey.forEach(v => {
          obj[v] = params[v]
        })
        this.singleDoubleEdit(obj)
      }
    },
    // 单双染新增
    async singleDoubleAdd(data = {}) {
