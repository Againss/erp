 ...
 * @LastEditors: admin
 * @LastEditTime: 2021-07-29 15:01:16
-->

<template>
  <div class="finishing-quotation">
    <div class="quotationDetail">
      ...
      <div class="base">
        ...
        <div id="dyeOrderDetailBranch" class="dyeOrderDetailBranch">
          ...
          <div v-for="(item, index) in baseDatas.pdDyeOrderDetailRespList" :key="index" class="list-order">
            <!-- 清单基本信息 -->
            <div class="list-form">
...
export default {
    ...
    const priceDataSource = {
      _price: {
        ...
        change: (value, v1, scope, v3, v4, index) => {
          ...
          scope.price = value
          let totalPlanQty = 0
          scope.orderDetailMaterialInfoRespList && scope.orderDetailMaterialInfoRespList.forEach((item, index) => {
            totalPlanQty += item.planQty
          })
          scope.totalPrice = this.$utils.getFloat(value * totalPlanQty, 2) || ''
        }
      },
      _totalPrice: {
        prop: 'totalPrice',
    ...
    const barConfig = {
      _confirm: {
        ...
        submit: async() => {
          ...
          const description = {
            beforeText: `在'成本中心-染整报价'更新了一条记录`,
        ...
        isShow: () => {
          // 待智布确认
          return this.status === 3 && !this.isRevise && !this.baseDatas.rejected && this.$permission(['costCenter:dye:confirm'])
        }
      ...
      _modify: {
        ...
        isShow: () => {
          // 待智布确认
          return this.status === 3 && !this.isRevise && this.$permission(['costCenter:dye:modifyPrice'])
        }
      ...
      _aduit: {
        ...
        isShow: () => {
          // 审批中可撤回
          return this.status === 5 && !this.isRevise && this.$permission(['costCenter:dye:audit'])
        }
      ...
      _withdraw: {
        ...
        isShow: () => {
          // 审批中可撤回
          return this.status === 5 && !this.isRevise && this.$permission(['costCenter:dye:withdraw'])
        }
      ...
      _back: {
        ...
        submit: () => {
          this.$router.push(`/cost-center/dyeing-finishing/list`)
        }
      }
    }
  ...
  methods: {
    ...
    async getDetail(data) {
      const res = await api.dyeingFinishingDetail(data)
      if (!res || res.code !== 200) {
        return false
      }
      ...
      this.baseDatas && this.baseDatas.pdDyeOrderDetailRespList.forEach((item, index) => {
        item.copytable = []
      })
      // 审批流程Id
      this.approveConfig.procInstId = this.detailData.processKey
      // this.approveConfig.proinstid = this.detailData.processKey
    ...
    showMore(item, index) {
      ...
      if (obj.pdDyeOrderDetailRespList[index].orderDetailMaterialInfoRespList.length === 0) {
        this.$message.error('暂无数据！')
        return
      }
      obj.pdDyeOrderDetailRespList[index].copytable = obj.pdDyeOrderDetailRespList[index].copytable.length ? [] : [...obj.pdDyeOrderDetailRespList[index].orderDetailMaterialInfoRespList]
      this.baseDatas = obj
    },
    // 审核
    appInfo(val) {
      ...
      this.$store.dispatch('app/fetchParamsLog', {
        ...
        description: {
          beforeText: '在成本中心染整报价审核' + str + '了一条数据',
          beforeCode: {
            inquiryNo: this.$route.query.id
          }
        },
...
.finishing-quotation{
  ...
  .list-order{
    ...
    // /deep/  .price-message .el-form-item__content {
    //   margin-left: 100;
    //   color:  #474747;
    // }
