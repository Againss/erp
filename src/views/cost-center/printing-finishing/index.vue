 ...
 * @LastEditors: admin
 * @LastEditTime: 2021-07-29 14:48:58
-->
<template>
  <div class="printing-finishing list">
    <div class="search">
      <cs-custom-form
        ref="searchForm"
      ...
      dataSource: [
        {
          ...
          isShow: (params) => {
            // 待询价状态不可以查看
            return params.row.status !== 1 && this.$permission(['costCenter:print:list'])
          },
          handle: (scope) => {
            this.$router.push({
              path: '/cost-center/printing-finishing/detail',
        ...
        },
        {
          ...
          isShow: (params) => {
            // 只有待智布确认状态有确认按钮
            return params.row.status === 3 && !params.row.rejected && this.$permission(['costCenter:print:confirm'])
          },
          handle: async(scope) => {
            this.$router.push({
              path: '/cost-center/printing-finishing/detail',
        ...
        },
        {
          ...
          isShow: (params) => {
            // 只有审批中可撤回
            return params.row.status === 5 && this.$permission(['costCenter:print:withdraw'])
          },
          handle: async(scope) => {
            this.$router.push({
              path: '/cost-center/printing-finishing/detail',
