...
export default {
  ...
  data() {
    ...
    let popOperatesDataSource = [
      { label: '编辑', permitTag: ['baseData:lossSetting:baseAddition:modify'], isShow: true, handle: scope => this.$nextTick(() => this.$refs['detailsPop'].init(scope.row)) },
      { label: '删除', permitTag: ['baseData:lossSetting:baseAddition:delete'], style: { 'color': '#FE4949' }, isShow: true, handle: scope => this.doDelete(scope.row) }
    ]
    ...
    return {
      ...
      columns: [
        ...
        { prop: 'cardingMethodInfo', label: label.cardingMethodInfo, minWidth: '120', showOverflowTooltip: true, formater: scope => scope.row.cardingMethodInfo ? scope.row.cardingMethodInfo.label : '-' },
        ...
        { prop: 'status', label: label.status, minWidth: '80', showOverflowTooltip: true,
          ...
          componentsOptions: {
            permitTag: 'baseData:lossSetting:baseAddition:status',
            changeStatus: (scope) => {
  ...
  methods: {
    ...
    async enableStatus(data = {}, scope) {
      ...
      if (code === 200) {
        ...
        this.fetchLog(
          this.getLogMessages('UPDATE', '/baseData/lossSetting/baseAddition/status', 'baseData'),
          { id: scope.row.id },
          JSON.stringify({
            beforeText: `在"基础数据-损耗设置-染损-额外加成"${scope.row.status === 'Y' ? '启用' : '禁用'}一条记录`,
    ...
    async del(row = {}) {
      ...
      if (code === 200) {
        this.fetchLog(this.getLog('DEL', '/baseData/lossSetting/baseAddition/delete'), { id: row.id }, JSON.stringify({ beforeText: `在'基础数据-损耗设置-染损-额外加成'删除一条记录`, beforeCode: { id: row.id }}))
