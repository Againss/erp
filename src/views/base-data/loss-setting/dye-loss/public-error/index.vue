...
export default {
  ...
  data() {
    ...
    let popOperatesDataSource = [
      { label: '编辑', permitTag: ['baseData:lossSetting:basePublicError:modify'], isShow: true, handle: scope => this.$nextTick(() => this.$refs['detailsPop'].init(scope.row)) },
      { label: '删除', permitTag: ['baseData:lossSetting:basePublicError:delete'], style: { 'color': '#FE4949' }, isShow: true, handle: scope => this.doDelete(scope.row) }
    ]
    ...
    return {
      ...
      columns: [
        ...
        { prop: 'publicErrorInfo', label: label.publicErrorInfo, minWidth: '150', showOverflowTooltip: true, align: 'right', formater: scope => scope.row.publicErrorInfo ? scope.row.publicErrorInfo.label : '-' },
        ...
        { prop: 'status', label: label.status, minWidth: '80', showOverflowTooltip: true,
          ...
          componentsOptions: {
            permitTag: 'baseData:lossSetting:basePublicError:status',
            changeStatus: (scope) => {
  ...
  methods: {
    ...
    async enableStatus(data = {}, scope) {
      ...
      if (code === 200) {
        ...
        this.fetchLog(
          this.getLogMessages('UPDATE', '/baseData/lossSetting/basePublicError/status', 'baseData'),
          { id: scope.row.id },
          JSON.stringify({
            beforeText: `在"基础数据-损耗设置-染损-附加走货公差"${scope.row.status === 'Y' ? '启用' : '禁用'}一条记录`,
    ...
    async del(row = {}) {
      ...
      if (code === 200) {
        this.fetchLog(this.getLog('DEL', '/baseData/lossSetting/basePublicError/delete'), { id: row.id }, JSON.stringify({ beforeText: `在'基础数据-损耗设置-染损-附加走货公差'删除一条记录`, beforeCode: { id: row.id }}))
