...
export default {
  ...
  data() {
    ...
    let popOperatesDataSource = [
      { label: '编辑', permitTag: ['baseData:lossSetting:baseDyeAttach:modify'], isShow: true, handle: scope => this.$nextTick(() => this.$refs['detailsPop'].init(scope.row)) },
      { label: '删除', permitTag: ['baseData:lossSetting:baseDyeAttach:delete'], style: { 'color': '#FE4949' }, isShow: true, handle: scope => this.doDelete(scope.row) }
    ]
    ...
    return {
      ...
      columns: [
        ...
        { prop: 'dyeTechInfo', label: label.dyeTechInfo, minWidth: '120', showOverflowTooltip: true, formater: scope => scope.row.dyeTechInfo ? scope.row.dyeTechInfo.label : '-' },
        ...
        { prop: 'status', label: label.status, minWidth: '80', showOverflowTooltip: true,
          ...
          componentsOptions: {
            permitTag: 'baseData:lossSetting:baseDyeAttach:status',
            changeStatus: (scope) => {
  ...
  methods: {
    ...
    async enableStatus(data = {}, scope) {
      ...
      if (code === 200) {
        ...
        this.fetchLog(
          this.getLogMessages('UPDATE', '/baseData/lossSetting/baseDyeAttach/status', 'baseData'),
          { id: scope.row.id },
          JSON.stringify({
            beforeText: `在"基础数据-损耗设置-染损-附加整理"${scope.row.status === 'Y' ? '启用' : '禁用'}一条记录`,
    ...
    async load(params = {}) {
      ...
      if (code === 200) {
        ...
        this.pagination = { ...this.pagination, dataTotal: total || 0, ...params, currentPage: pageNum, pageSize: pageSize, page: pageNum }
      }
    },
    ...
    async del(row = {}) {
      ...
      if (code === 200) {
        this.fetchLog(this.getLog('DEL', '/baseData/lossSetting/apsDyeAttach/delete'), { id: row.id }, JSON.stringify({ beforeText: `在'基础数据-损耗设置-染损-附加整理'删除一条记录`, beforeCode: { id: row.id }}))
