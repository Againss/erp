...
export default {
  ...
  data() {
    ...
    let popOperatesDataSource = [
      { label: '编辑', permitTag: ['baseData:lossSetting:basePrintBasic:modify'], isShow: true, handle: scope => this.$refs['detailsPop'].init(scope.row) },
      { label: '删除', permitTag: ['baseData:lossSetting:basePrintBasic:delete'], style: { 'color': '#FE4949' }, isShow: true, handle: scope => this.doDelete(scope.row) }
    ]
    ...
    return {
      ...
      columns: [
        ...
        { prop: 'reqPrintInfo', label: label.reqPrintInfo, minWidth: '120', showOverflowTooltip: true, formater: scope => scope.row.reqPrintInfo ? scope.row.reqPrintInfo.label : '-' },
        { prop: 'weightRange', label: label.weightRange + '(kg)', minWidth: '120', showOverflowTooltip: true, align: 'right', formater: scope => `${scope.row.weightIni}〜${scope.row.weightEnd}` },
        { prop: 'wastage', label: label.wastage, minWidth: '80', showOverflowTooltip: true, align: 'right', formater: scope => this.$filters.numberToPercentage(scope.row.wastage) + '%' },
        { prop: 'shearWastage', label: `${label.shearWastage}(kg)`, minWidth: '120', showOverflowTooltip: true, align: 'right' },
  ...
  methods: {
    ...
    async enableStatus(data = {}, scope) {
      ...
      if (code === 200) {
        ...
        this.fetchLog(
          this.getLogMessages('UPDATE', '/baseData/lossSetting/basePrintBasic/status', 'baseData'),
          { id: scope.row.id },
          JSON.stringify({
            beforeText: `在"基础数据-损耗设置-印损-基础设置"${scope.row.status === 'Y' ? '启用' : '禁用'}一条记录`,
    ...
    async del(row = {}) {
      ...
      if (code === 200) {
        this.fetchLog(this.getLog('DEL', '/baseData/lossSetting/basePrintBasic/delete'), { id: row.id }, JSON.stringify({ beforeText: `在'基础数据-损耗设置-印损-基础设置'删除一条记录`, beforeCode: { id: row.id }}))
