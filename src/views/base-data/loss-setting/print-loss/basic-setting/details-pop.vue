...
<script>
import { add, detail, modify, printCraftSelect } from './api/index.js'
...
export default {
  ...
  data() {
    ...
    return {
      options: {
        ...
        formOptions: {},
        content: {
          reqPrintInfo: { itemType: 'select', prop: 'reqPrintInfo', filterable: true, label: label.reqPrintInfo, valueType: 'object', disabled: false, clearable: true, rules: [commonBlurReg],
            dataSource: [], visibleChange: (value) => {
              if (value) {
                this.getReqPrintInfo()
              }
            }
          },
          ...
          shearWastage: { itemType: 'input', prop: 'shearWastage', label: label.shearWastage, maxlength: 30, disabled: false, clearable: true, rules: [commonBlurReg, max999Regtype], suffix: 'kg', placeholder: '最多输入10位数字' }
        }
      }
    }
  },
  methods: {
    // 获取印染类型 下拉
    async getReqPrintInfo() {
      const { code = 0, data = [] } = await printCraftSelect({ enabled: 'Y' })
      if (code === 200) { // 表示成功
        this.options.content['reqPrintInfo'].dataSource = data.map(n => ({ label: n.name, value: n.uuid }))
      }
    },
    // 通过id获取信息
    ...
    // 更新数据
    async modify(row) {
      const { code = 0, message = '', data = {}} = await modify({ ...row, wastage: this.$filters.percentageToNumber(Number(row.wastage)), weightIni: Number(row.weightIni), weightEnd: Number(row.weightEnd), shearWastage: Number(row.shearWastage) })
      if (code === 200) { // 表示成功
    ...
    // 新增数据
    async add(row) {
      const { code = 0, message = '', data = {}} = await add({ ...row, wastage: this.$filters.percentageToNumber(Number(row.wastage)), weightIni: Number(row.weightIni), weightEnd: Number(row.weightEnd), shearWastage: Number(row.shearWastage) })
      if (code === 200) { // 表示成功
        this.$emit('add', { code, message, data })
        this.options.visible = false // 关闭弹框
      }
    },
    init(row = { reqPrintInfo: {}}) {
      this.options.visible = true // 显示弹框
      this.$nextTick(() => { // 在下一次事件循环中处理
        this.options.content['reqPrintInfo'].dataSource = []
        this.options.formDatas = {}
        if (row.id) { // 存在id表示编辑
          this.options.title = '编辑'
          this.options.content['reqPrintInfo'].dataSource.push(row.reqPrintInfo)
