...
-->
<template>
  <div class="countryArea countryArea-fabric">
    <!-- 新增菜单按钮 -->
    <!--     <div class="button button-fabric">-->
...
export default {
  ...
  data() {
    ...
    let popOperatesDataSource = [
      { label: '编辑', permitTag: ['baseData:lossSetting:baseWeaveWastage:modify'], isShow: true, handle: scope => this.$nextTick(() => this.$refs['detailsPop'].init(scope.row)) },
    ...
    return {
      ...
      columns: [
        ...
        { prop: 'fabrications', label: label.fabrications, minWidth: '120', showOverflowTooltip: true },
  ...
  methods: {
    ...
    async enableStatus(data = {}, scope) {
      ...
      if (code === 200) {
        ...
        this.fetchLog(
          this.getLogMessages('UPDATE', '/baseData/lossSetting/baseFabrications/status', 'baseData'),
          { id: scope.row.id },
          JSON.stringify({
            beforeText: `在"基础数据-损耗设置-织损-布种"${scope.row.status === 'Y' ? '启用' : '禁用'}一条记录`,
    ...
    async load(params = {}) {
      ...
      if (code === 200) {
        ...
        this.pagination = { ...this.pagination, dataTotal: total || 0, ...params, currentPage: pageNum, pageSize: params.pageSize, page: pageNum }
      }
    },
    ...
    doDelete(row) {
      this.$confirm(' 确认删除当前数据吗？', '提示', { confirmButtonText: '确定', cancelButtonText: '取消', type: 'warning' }).then(async() => {
        ...
        this.fetchLog(
          this.getLog('DEL', '/baseData/lossSetting/baseFabrications/delete', 'baseData'),
          { id: row.id },
          JSON.stringify({
            beforeText: `在'基础数据-损耗设置-织损-布种'删除一条记录`,
