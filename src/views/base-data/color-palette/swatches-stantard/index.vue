...
<template>
  <div class="fabricTypes">
    ...
    <div class="button">
      <el-button
        v-permission="['customerOrder:colorModel:colorModelStandardConf:add']"
        size="small"
        icon="el-icon-plus"
        type="primary"
        @click="addClick"
  ...
  data() {
    ...
    const columns = [
      ...
      },
      {
        ...
        componentsOptions: {
          permitTag: 'customerOrder:colorModel:colorModelStandardConf:isEnable',
          changeStatus: (scope) => {
    ...
    let popOperatesDataSource = [
      {
        label: '审核',
        permitTag: ['customerOrder:colorModel:colorModelStandardConf:approve'],
        isShow: (scope) => {
        ...
        handle: scope => {
          this.$confirm('确认审核当前数据吗？', '提示', {
            ...
            .then(() => {
              this.approveSwatchesStantard(scope.row.id)
            })
            .catch(() => {
              this.$message({
                type: 'info',
      ...
      {
        label: '编辑',
        permitTag: ['customerOrder:colorModel:colorModelStandardConf:modify'],
        isShow: true,
        handle: scope => {
      ...
      },
      {
        label: '删除',
        style: { 'color': '#FE4949' },
        permitTag: ['customerOrder:colorModel:colorModelStandardConf:delete'],
        isShow: (scope) => {
          return scope.row.status === 'N'
        },
        handle: scope => {
          this.$confirm('确认删除当前数据吗？', '提示', {
            ...
            .then(() => {
              this.deleteSwatchesStantard(scope.row.id)
            })
            .catch(() => {
              this.$message({
                type: 'info',
    ...
    const popOptions = {
      ...
      okText: '保存',
      ok: params => {
        this.swatchesStantardOption(params)
      },
  ...
  methods: {
    ...
    async enableStatus(data = {}) {
      ...
      if (res.code === 200) {
        ...
        this.fetchLog(
          this.getLogMessages('UPDATE', '/customerOrder/colorModel/colorModelStandardConf/isEnable', 'customerOrder'),
          { id: data.id },
          JSON.stringify({
            beforeText: `在"客户订单-基础数据-色板标准"${data.isEnable === 'Y' ? '启用' : '禁用'}一条记录`,
    ...
    addClick() {
      ...
      this.$set(this.content[0], 'disabled', false)
      this.$set(this.content[2], 'disabled', false)
      this.PopDialogHandle('新增色板标准')
      this.$set(this.popOptions, 'content', this.content)
      this.$set(this.popOptions, 'formDatas', {})
    },
    // 新增、编辑色板标准
    swatchesStantardOption(params) {
      if (this.type === 'add') {
        const obj = {}
        ...
        this.swatchesStantardEdit(obj)
      }
    },
    // 色板标准新增
    async swatchesStantardAdd(data = {}) {
      ...
      this.fetchLog(
        this.getLogMessages('INSERT', '/customerOrder/colorModel/colorModelStandardConf/add', 'customerOrder'),
        data,
        JSON.stringify({
          beforeText: `在'客户订单-基础数据-色板标准'新增一条记录`,
          beforeCode: data
        })
      )
      this.getListData()
    ...
    // 色板标准编辑
    async swatchesStantardEdit(data = {}) {
      ...
      flag && this.fetchLog(
        this.getLogMessages('UPDATE', '/customerOrder/colorModel/colorModelStandardConf/modify', 'customerOrder'),
        data,
        JSON.stringify({
          beforeText: `在'客户订单-基础数据-色板标准'将${JSON.stringify({ 'id': data.id })}`,
          afterText: '修改为',
    ...
    // 审核色板标准
    async approveSwatchesStantard(id) {
      ...
      // this.fetchLog(
      //   this.getLogMessages('UPDATE', '/customerOrder/colorModel/colorModelStandardConf/approve', 'customerOrder'),
      //   { id: id },
      //   JSON.stringify({
      //     beforeText: `在'客户订单-基础数据-色板标准'审核一条记录`,
      //     beforeCode: { id: id }
      //   })
      // )
      this.getListData({ pageNum: this.pagination.pageNum || 1, pageSize: this.pagination.pageSize || 20 })
    ...
    // 删除色板标准
    async deleteSwatchesStantard(id) {
      ...
      this.fetchLog(
        this.getLogMessages('DEL', '/customerOrder/colorModel/colorModelStandardConf/delete', 'customerOrder'),
        { id: id },
        JSON.stringify({
          beforeText: `在'客户订单-基础数据-色板标准'删除一条记录`,
          beforeCode: { id: id }
        })
      )
      this.getListData({ pageNum: this.pagination.pageNum || 1, pageSize: this.pagination.pageSize || 20 })
