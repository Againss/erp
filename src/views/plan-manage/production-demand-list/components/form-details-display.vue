    ...
    <template v-if="dataSource.length">
      <div v-for="(item, index) in dataSource" :key="index" class="form-details-display-item">
        <div class="headerList" :class="item.expandFlag ? 'expand' : 'notExpand'">
          <el-checkbox v-if="checkShow" v-model="item.checked" class="checkbox" @change="option.checkboxButtonClick" />
          ...
          <div class="headerList-content">
            <div v-for="(headerItem, headerIndex) in headerList" :key="headerIndex" class="headerList-content-item">
              <label class="label">{{ headerItem.label }}</label>
          ...
          <el-button v-if="rightButtonShow" class="right-button" submit type="primary" plain size="small" @click="option.rightButtonClick(item, index)">{{ rightButtonText }}</el-button>
        </div>
        <div class="contentList">
          <cs-custom-form ref="searchForm" :data-source="[{...customFormData[0], spanMethod: ({ row, column, rowIndex, columnIndex }) => {return spanMethod({ row, column, rowIndex, columnIndex }, item.apsProduceRequireProcessInfoRespList || [])}, dataSource: item.apsProduceRequireProcessInfoRespList || []}]" :options="formOtions" />
        </div>
      <!-- <div class="contentList">
