    ...
    <template v-if="dataSource.length">
      <div v-for="(item, index) in dataSource" :key="index" class="form-details-display-item">
        ...
        <div class="contentList">
          <cs-custom-form
            ref="searchForm"
            :data-source="[{...customFormData[0], spanMethod: ({ row, column, rowIndex, columnIndex }) => {return spanMethod({ row, column, rowIndex, columnIndex }, item.apsProduceRequireProcessPageRespList || [])}, dataSource: item.apsProduceRequireProcessPageRespList || []}]"
            :options="formOtions"
          />
        </div>
      <!-- <div class="contentList">
